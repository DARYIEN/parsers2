   ...
   public function launch($extUrl, $extVector, $extTimeout = null)
    ...
    private function setUserAgent()
    {
        ...
        $agentOS = array(
			...
			$countAB = count($agentBrowser) -1;
			$countAOS = count($agentOS)-1;
        //randomly generate UserAgent
        $this->userAgent = $agentBrowser[rand(0,$countAB)].'/'.rand(1,8).'.'.rand(0,9).' (' .$agentOS[rand(0,$countAOS)].' '.rand(1,7).'.'.rand(0,9).'; en-US;)';
    }
    ...
    private function setCurl()
    {
			try
			{
				...
				if($this->returnData === false)
				{
					throw new Exception( 'Ошибка curl: ' . curl_error($action));
				}
				curl_close($action);
			}
			catch(Exception $e)
