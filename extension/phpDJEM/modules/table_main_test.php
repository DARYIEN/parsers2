...
foreach ($vids as $vid_item) {
    if ($vid_item->_type != 221) {
        $path_item = $path;
    } else {
        $path_item = $vid_item->_path;
    }
    ...
    foreach ($query as $item) {
        ...
        if ($vid != R('DJEM')->Load($item->_parent_id)->_name) {
            ...
            if (R('DJEM')->Load($item->_parent_id)->_type == 221)
                echo "</div><div class='block'><p style='font-size: 15px; font-weight: bold;font-family: PT Sans,Arial;'>" .R('DJEM')->Load($id)->_name . " $vid</p>"; 
        }
        
        if ($subheading!=preg_replace("/(х.*)$/","",$item->_name)) echo "</div><div class='block'><p style='font-size: 15px; font-weight: bold;font-family: PT Sans,Arial;'>$vid " .preg_replace("/(х.*)$/","",$item->_name) . " </p>";
$subheading =preg_replace("/(х.*)$/","",$item->_name);
...
$sbj->Path('main.Advertisement.$')->Where("_name like '" . R('DJEM')->Load($id)->
    msort . "' || msort like '" . R('DJEM')->Load($id)->msort . "'")->limit(5)->
    sort('random');
foreach ($sbj as $it) {
    echo "<p style='margin:7px 0' ><a href='/advertisement/bulletin_board.phtml?cont=$it->_id'>$it->_name</a></p>";
}
if (!count($sbj))
    echo "Обьявлений для этого раздела нет.<br>"
