...
<input href="javascript:void(0);" value="Введите размер" onfocus="if (value == 'Введите размер') {value =''}" path="<?php echo htmlspecialchars((isset($_REQUEST['par2'])?$_REQUEST['par2']:'')); ?>" city="<?php echo $city; ?>" type="text"  onclick="d($(this))" class="slct2 sin" style="background:url('/image/new_arrow2.png') no-repeat; border: medium none;"  />
...
$foreach_v__count = 0; foreach ($foreach__v as $foreach_v) { ++$foreach_v__count; ?>
		...
		<li  onclick=" m($(this)); $('.sin').val('<?php echo $foreach_v->{'_name'}; ?>');  "><a level="<?php echo $foreach_v->{'_level'}; ?>" value="<?php echo $foreach_v->{'_path'}; ?>" tip="<?php echo $foreach_v->{'_type'}; ?>" ><?php echo $foreach_v->{'_name'}; ?></a></li>
	...
	<?php
	  function for_csv($v) {
    $for_csv=str_replace(array(")","(",".","/"),"",str_replace(array(" "," "),"_",$v));
    return $for_csv;
}
	$coco = R('DJEM')->Load($_REQUEST["par2"])->_id;
	if (R('DJEM')->Load($_REQUEST["par2"])->_name!=R('DJEM')->Parent($coco,5)->_name)$vvid="/".for_csv(R('DJEM')->Parent($coco,5)->_name);
	
	?>
	<a style="text-decoration:none;"  id="zhamk" href="/prodazha/<?php echo str_replace(".phtml","",R('DJEM')->Parent($coco,4)->_url).$vvid."/".for_csv(R('DJEM')->Load($_REQUEST["par2"])->_name); ?>">Найти</a>
