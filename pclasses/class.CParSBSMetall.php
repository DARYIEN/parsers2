...
class CParSBSMetall extends CParMain{
    var $city_id;
    static $name_parser = array(
        'sbsmetall' => 'СБС металл'
    );
    public $items = array();
    function start(){
    ...
    function __construct(){
        ...
        $this->document_urls['main'] = 'http://sbs-metall.ru/images/stories/kopiyaaza.xls';
        //$this->coef = 1000;
        $this->dual_cost = false;
        $this->decimal = true;
        $this->price_id = 12267574;
        $this->author = 'Михаил';
    }
    ...
    function documentParsing(){
        ...
        foreach($this->sheet as $rows){
            ...
            if(!empty($name)&& !empty($costs)){
                $this->items[] = array('name' => preg_replace('/Ø/iu','',preg_replace('/\?/iu','', preg_replace('/;/iu','!', preg_replace('/\s+/iu',' ',trim($header.' '.$treeString.' '.$name.' '.$this->filter['end']))))), 'cost' => $costs);
            }
            $name = '';
            $cost = '';
            unset($costs);
