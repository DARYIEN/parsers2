 ...
 * Time: 12:44
 */
class CParDiposKuban extends CParMain{
    var $city_id;
    var $message = '';
    var $document_urls;
    var $home_url;
    var $company_name;
    public $temp = array();
    public $stringForFind = 'div.tab_metproduct div.tabline';
    static $name_parser = array(
        'diposkuban' => 'Дипос-кубань'
    );
    function start(){
        $this->getDocuments();
        $this->processParsing();
    ...
    function __construct(){
        ...
        $this->dual_cost = true;
        $this->home_url = 'http://dipos-kuban.ru/metal_production/2172/';
        $this->document_urls = $this->getUrl();
        $this->price_id = 10928483;
        $this->iconv = true;
    }
    ...
    function processParsing(){
        ...
        foreach($this->document_list as $path){
            $this->filter = array(
                'propusk' => array(2,3,4,6,7,8,9,10,11),
                'cost' => array(5),
            );
            //$this->stringForFind = '';
    ...
    function documentParsing(){
        ...
        foreach($this->temp as $items){
            ...
            foreach($items as $key => $item){
                ...
                if($item == '&lt;Свойства не назначены&gt;') continue;
            ...
            $temp['cost'] = $cost;
            $temp['name'] = preg_replace('/Ǿ/iu','',preg_replace('/\?/iu','', preg_replace('/;/iu','!', preg_replace('/\s+/iu',' ', preg_replace('/&nbsp;/iu',' ', str_replace(array('с ост., имп.', 'c ,', 'с ост.','ост.','имп.'),'',$name))))));
    ...
    public function getUrl(){
        ...
        $info_all = $parse->find('ul.leftmenu li a');
        $href = array();
        foreach($info_all as $info_first){
            $href[] = 'http://dipos-kuban.ru'.$info_first->href;
        }
        $href[] = $this->home_url;
