...
class CParMetAktive extends CParMain{
    var $city_id;
    static $name_parser = array(
        'MetAktive' => 'Металл-актив'
    );
    public $items = array();
    function start(){
    ...
    function __construct(){
        ...
        $this->document_urls['main'] = 'http://www.metall-aktiv.ru/metal-active-price.xls';
    ...
    function documentParsing(){
        ...
        foreach($this->sheet as $rows){
            ...
            if(!empty($name)&& !empty($costs)){
                $this->items[] = array('name' => preg_replace('/Ǿ/iu','',preg_replace('/\?/iu','', preg_replace('/;/iu','!', preg_replace('/\s+/iu',' ',iconv('CP1251','UTF-8',iconv('UTF-8','CP1252',trim($header.' '.$treeString.' '.$name.' '.$this->filter['end']))))))), 'cost' => $costs);
            }
            $name = '';
            $cost = '';
            unset($costs);
