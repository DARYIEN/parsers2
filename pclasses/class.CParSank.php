...
class CParSank extends CParMain{
    var $city_id;
    var $message = '';
    var $document_urls;
    var $home_url;
    ...
    function __construct(){
        ...
        $this->document_urls = array(   'http://sanc.spb.ru/assortiment_print.php',
                                    );
        $this->home_url = current($this->document_urls);
        $this->price_id = 8438131;
        $this->iconv = true;
    }
    ...
    function documentParsing($path){
        ...
        $headers = $parse->find('table td table');
        $tables = $parse->find(' table td table');
        if(!empty($headers)){
            foreach($headers as $header){
                foreach($header->find('td b u') as $content){
                    $text[]=trim($name = iconv('UTF-8','windows-1251',$content->plaintext));
            ...
            foreach($tables as $key=>$table){
                ...
                foreach($table->find('tr') as $tr){
                    ...
                    foreach($tr->find('td') as $td){
                        ...
                        //if(in_array(strip_tags($td->plaintext),$text)) $headerNum++;
                        if(count($tr->find('td')) == 1) $headerNum++;
                        //if(in_array($td->find('strong',0)->plaintext,$text)) $headerNum++;
                        if($step == 1){
                            //continue 2;
                        }else if ($step == 2){
                            $name = $text[$headerNum].' '.$td->innertext;
                        }else if ($step >= 3 && $step <= $end_cell['stop_cell']){
                            //$name .= ' '.$td->innertext;
                        }else if ($step == $end_cell['cost_cell1']){
                            if (!empty($td->innertext))
                                $cost = str_replace(' ', '',trim(strip_tags($td->innertext)));
                   ...
                   // }
                    ...
                    $name = str_replace(';','!', $name);
                    //$name = str_replace(' ',' ',preg_replace('~&nbsp;+~','',$name));
