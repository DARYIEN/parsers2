...
class CParBaltTrade extends CParMain{
    var $city_id;
    ...
    function documentParsing(){
        ...
        foreach($this->sheet as $rows){
            ...
            foreach($rows as $num_row => $row){
                if(!empty($row)){
                    if(in_array(trim($row), array('Цена 1т',))){
                    ...
                    $name .= ' '.$rows[$this->filter['horizontal']['from']['numeric']];
                }
            }
    ...
    public function getUrl(){
        $parse = file_get_html('http://www.balttreyd.ru/price');
        $link = $parse->find('div#center-box p a', 0);
        return 'http://www.balttreyd.ru'.$link->href;
    }
}