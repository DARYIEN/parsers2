...
class CParPiloTorg extends CParMain{
    var $city_id;
        ...
        foreach($this->sheet as $rows){
            ...
            if(!empty($name)&& !empty($costs)){
                ...
                if(isset($this->filter['fake'])){
                    $fake = $this->filter['fake'];
                }
                $this->items[] = array('name' => preg_replace('/Ǿ/iu','',preg_replace('/\?/iu','', preg_replace('/;/iu','!', preg_replace('/\s+/iu',' ',trim($fake.' '.$header.' '.$treeString.' '.$name.' '.$this->filter['end']))))), 'cost' => $costs);
            }
            $name = '';
            $cost = '';
            unset($costs);
    ...
    public function getUrl(){
        $parse = file_get_html('http://stroymaterialy.ru.com/pages/nashi-ceny');
        $link = $parse->find('div#content p a', 0);
        return 'http://stroymaterialy.ru.com'.$link->href;
    }
}