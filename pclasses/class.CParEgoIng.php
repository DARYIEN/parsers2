...
class CParEgoIng extends CParMain{
    var $city_id;
    static $name_parser = array(
        'EgoIng' => 'EgoIng'
    );
    public static $kursUSD;
    public static $kursEUR;
    public $items = array();
    function start(){
    ...
    function __construct(){
        ...
        $this->document_urls = $this->getUrl('http://www.egoing.ru/price-list/');
    ...
    function documentParsing(){
        ...
        foreach($this->sheet as $rows){
            ...
            foreach($rows as $num_row => $row){
                if(!empty($row)){
                    if(count($rows) > 2 && isset($rows[7]) && trim($rows[7]) != false){
                        $this->filter['currency'] = trim($rows[7]);
                    }else{
                        $this->filter['currency'] = 'RUR';
                    }
                    if(in_array($row, array('м'))){
                        $this->filter['coef'] = 1000;
                        $this->filter['end'] = '';
                    }
                    if(in_array($row, array('шт'))){
                        $this->filter['coef'] = 1;
                        $this->filter['end'] = 'шт';
                    }
                    ...
                    if($this->filter['currency'] == 'RUR') $currencyCoef = 1;
                    if($this->filter['currency'] == 'USD') $currencyCoef = self::$kursUSD;
                    if($this->filter['currency'] == 'EUR') $currencyCoef = self::$kursEUR;
