...
class CParSteelUnion extends CParMain{
    var $city_id;
    static $name_parser = array(
        'steelunion' => 'Сталь юниун'
    );
    public $items = array();
    function start(){
    ...
    function processParsing(){
        foreach($this->document_list as $path){
            ...
            $this->filter =  array(
                ...
                'coef' => 1,
                'end' => '',
                'header' => 'Лист легированные (09Г2С)',
                'beginOfPrice' => 0,
                'skleyka' => false,
                'dualheader'=>false,
                'horizontal' => array(
            ...
            foreach($rows as $num_row => $row){
                if(!empty($row)){
                    ...
                    if(count($rows) == 1){
                        ...
                        if ($this->filter['header']){$header = $row; }else {$header = $this->filter['header'].' '.$row;}
                        $new_header=true;
                        continue;
                    }
    ...
    public function getUrl(){
        $parse = file_get_html('http://www.steelunion.ru/price.html');
        $link = $parse->find('div#toright a#downlink', 0);
        return $link->href;
    }
}