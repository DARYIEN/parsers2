...
class CParMEtOptTrade extends CParMain{
    var $city_id;
    var $message = '';
    var $document_urls;
    var $home_url;
    ...
    function __construct(){
        ...
        $this->dual_cost = true;
        $this->author = 'Феликс';
        $this->document_urls['al_list'] = 'http://www.metopttrade.ru/produkcziya/alyuminievyij-list/';
        $this->document_urls['list_rifl'] = 'http://www.metopttrade.ru/produkcziya/list-riflenyij-kvintet/';
        $this->document_urls['al_rulon'] = 'http://www.metopttrade.ru/aluminieviy-rulon/';
        $this->document_urls['al_plita'] = 'http://www.metopttrade.ru/produkcziya/alyuminievaya-plita/';
        $this->document_urls['al_prut'] = 'http://www.metopttrade.ru/produkcziya/alyuminievyij-prutok/';
        ...
        $this->document_urls['alum_polosa'] = 'http://www.metopttrade.ru/produkcziya/polosa-alyuminievaya/';
    ...
    function processParsing(){
        foreach($this->document_list as $key => $path){
            if($key == "al_list"){
                $this->filter = array('cost' => [3,4], 'hide' => array(1,2), 'coef' => 1000, 'selector' => 'div.content table');
            ...
            if($key == "al_plita"){
                $this->filter = array('cost' =>[1,2], 'hide' => array(), 'coef' => 1000, 'selector' => 'div.content table');
                $this->documentParsing($path);
            }
            ...
            if($key == "truba_pryam"){
                $this->filter = array('cost' => [4,5], 'hide' => array(2,3), 'coef' => 1000, 'selector' => 'div.content table');
                $this->documentParsing($path);
            }

            if($key == "profil_alum"){
                $this->filter = array('cost' => [4,5], 'hide' => array(1,2,3), 'coef' => 1000, 'selector' => 'div.content table');
                $this->documentParsing($path);
            }
            ...
            if($key == "profil_nat_pot"){
                $this->filter = array('cost' => [2], 'hide' => array(0), 'coef' => 1, 'selector' => 'div.content table', 'dop' => ' п.м.');
                $this->documentParsing($path);
            }

            if($key == "ohlad"){
                $this->filter = array('cost' => [4], 'hide' => array(0,2,3), 'coef' => 1000, 'selector' => 'div.content table');
                $this->documentParsing($path);
            }
            //unlink($path);
        }
