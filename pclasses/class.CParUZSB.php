...
class CParUZSB extends CParMain{
    var $city_id;
    var $message = '';
    var $document_urls;
    var $home_url;
    ...
    static $name_parser = array(
        'UZSB' => 'УРАЛЬСКИЙ ЗАВОД СВАРНОЙ БАЛКИ'
    );
    function start(){
        $this->getDocuments();
        $this->processParsing();
    ...
    function __construct(){
        ...
        $this->document_urls = array(   'http://www.uzsb.ru/uzsb/balka/balka_stock/',
                                    );
        $this->home_url = current($this->document_urls);
        $this->price_id = 10655836;
        $this->iconv = true;
    }
    ...
    function documentParsing($path){
        ...
        //$headers = $parse->find('div.myTablePrice');
        $tables = $parse->find('#content table');
       /* if(!empty($headers)){
                ...
                foreach($table->find('tr') as $tr){
                    ...
                    foreach($tr->find('td') as $td){
                        ...
                        if($step == 2){
                            ...
                            $cost = str_replace(' ','',preg_replace('~[^0-9]+~','',$cost));
                            $cost = rtrim($cost,'00');
