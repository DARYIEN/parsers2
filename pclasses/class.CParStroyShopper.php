...
class CParStroyShopper extends CParMain{
    var $city_id;
    var $message = '';
    var $document_urls;
    var $home_url;
    ...
    function __construct(){
        ...
        $this->dual_cost = true;
        $this->author = 'Михаил';
        $this->document_urls = array(   'http://www.stroyshopper.ru/price/',
                                    );
            ...
            foreach($tables as $key=>$table){
                ...
                foreach($table->find('tr') as $tr){
                    ...
                    foreach($tr->find('td') as $key =>  $td){
                        if(count($tr->find('td')) > 1 && $key == 1) {
                            $cost = str_replace(array(' ', 'руб.','руб', 'р.', '&#160;', '&nbsp;', chr(160), 'От', 'от'), '', $td->plaintext);
