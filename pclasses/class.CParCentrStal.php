...
class CParCentrStal extends CParMain{
    var $city_id;
    static $name_parser = array(
        'centrstal' => 'ЦентрСталь'
    );
    function start(){
    ...
    function __construct(){
        ...
        $this->document_url = 'http://www.centrstal.ru/price.xls';
        //$this->coef = 1000;
        $this->dual_cost = false;
        $this->price_id = 7522027;
    }
            ...
            foreach($rows as $num_row => $row){
                if(!empty($row)){
                    if(in_array($num_row, array($this->filter['horizontal']['to']['numeric']))){
                        $cost = (int) str_replace(array(' ','руб.'),'',$row);
                    ...
                    if(stripos($header ,"Катанка") != false && stripos($row,'Круг') !=false){$header='Круг';}
                    if($num_row == $this->filter['horizontal']['from']['numeric'] ) $name =$header;

                    $name .= ' '.$row;
                }
            }
            if(!empty($name)&& !empty($cost)){
                $this->items[] = array('name' => str_replace(array('=','*'),'',preg_replace('/\?/iu','', preg_replace('/;/iu',',', preg_replace('/\s+/iu',' ', $name)))), 'cost' => $cost);
            }
            $name = '';
            $cost = '';
