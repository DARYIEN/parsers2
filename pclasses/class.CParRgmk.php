 ...
 * Time: 12:44
 */
class CParRgmkCR extends CParMain{
    var $city_id;
    var $message = '';
    var $document_urls;
    var $home_url;
    ...
    static $name_parser = array(
        'rgmk' => 'Русская горно-металлургическая компания центральный регион'
    );
    function start(){
        $this->getDocuments();
        $this->processParsing();
    ...
    function __construct(){
        ...
        $this->company_name = 'Русская горно-металлургическая компания ЦР'.current(array_values($this->cities_list[$this->city_id]));
        $this->formDirsArray()->createDirs();
        $this->document_extended = '.html';
        $this->home_url = 'http://rgmk-cr.ru/catalog/armatura/';
    ...
    function documentParsing($path){
        ...
        $table = $parse->find('table#table-of-products tbody tr.product');
        if(!empty($table)){
            $result = array();
            foreach($table as $tr){
                $temp = array();
                foreach($tr->find('td.price') as $cost){
                    $temp['cost'] = str_replace(' ', '',$cost->innertext);
                }
                foreach($tr->find('td.name, td.size, td.width') as $td){
                    //p($td->innertext);
                ...
                $temp['cost'] = preg_replace('/\s+/i',' ', $temp['cost']);
                $result[] = $temp;
            }
    ...
    public function getUrl(){
        ...
        $info_all = $parse->find('div#catalog-navigation ul li');
        $href = array();
        foreach($info_all as $info_first){
            $info_second = $info_first->find('a',0);
            $href[] = 'http://rgmk-cr.ru'.$info_second->href;
            //break;
        }
