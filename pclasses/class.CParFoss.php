...
class CParFoss extends CParMain{
    var $city_id;
    ...
    function __construct(){
        ...
        $this->document_url = 'http://voss-metall.ru/images/price/price-list-foss-metall.xls';
        $this->coef = 1000;
        $this->dual_cost = false;
        $this->price_id = 7669656;
    }
    ...
    function documentParsing(){
        ...
        foreach($this->sheet as $rows){
            ...
            foreach($rows as $num_row => $row){
                if(!empty($row)){
                    if(in_array($row, array('метр')) || in_array($row, array('штука')) || $num_row == $this->filter['horizontal']['from']['numeric']){
                        continue;
                    }
                    if($num_row == $this->filter['horizontal']['to']['numeric'] - 1){
                        continue;
                    }
                    if($num_row == $this->filter['horizontal']['to']['numeric'] - 2){
                        continue;
                    }
