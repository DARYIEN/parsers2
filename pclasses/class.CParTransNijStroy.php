...
class CParTransNijStroy extends CParMain{
    var $city_id;
    var $message = '';
    var $document_urls;
    var $home_url;
    ...
    function __construct(){
        ...
        $this->document_urls = array('http://xn----ftbtrcjkce.xn--p1ai/price_armatura.html');
        $this->home_url = current($this->document_urls);
        $this->price_id = 8438002;
        $this->iconv = true;
    }
    ...
    function documentParsing($path){
        ...
        $headers = $parse->find('td.xl81, td.xl79, td.xl73, td.xl75, td.xl72');
        $tables = $parse->find('table');
        if(!empty($headers)){
            foreach($headers as $header){
                    $text[]=str_replace(array('3,ГОСТ 14637-89 ст3сп/пс','Производство ЗСМК:','Производство ДМЗ им. Петровского, ТУ У 14-2-1185-97 ст. 3:'),array('','ШВЕЛЛЕР ГОСТ 8240-97 ст.3','ШВЕЛЛЕР ГОСТ 8240-97 ст.3'),trim(preg_replace('/\s+/iu',' ', $header->plaintext)));
                }
        }
            //p($text);

        $cell=array(
            'standart'=>array('stop_cell'=>3, 'cost_cell1'=>4,),
        );
        ...
        if(!empty($tables)){
            ...
            foreach($tables as $key=>$table){
                ...
                foreach($table->find('tr') as $tr){
                    ...
                    //}
                    if (count($tr->find('td')) == 2 && $step != 1) $step = 3;
                    foreach($tr->find('td.xl67,td.xl70,td.xl71,td.xl68,td.xl69,td.xl71') as $td){
                        if($td->plaintext == '-'){
                            $step++;
                            continue;
                        }
