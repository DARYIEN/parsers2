...
class CParKazTemirKontraktAlmaty extends CParMain{
    var $city_id;
    static $name_parser = array(
        'kaztemirkontrakt' => 'Казтемирконтракт Алматы'
    );
    public $items = array();
    function start(){
    ...
    function __construct(){
        ...
        $this->document_url = 'http://kaztemirkontrakt.kz/images/price/price-almaty.xlsx';
        //$this->coef = 1000;
        $this->dual_cost = true;
        $this->price_id = 11108112;
        $this->author = 'Феликс';
    }
        ...
        foreach($this->sheet as $rows){
            ...
            foreach($rows as $num_row => $row){
                if(!empty($row)){
                    ...
                    if($this->filter['header']&&$num_row == $this->filter['horizontal']['from']['numeric'] ) $name =str_replace('ВГП, ЭСВ','',$header.' '.$steel);
                    $new_header=false;
