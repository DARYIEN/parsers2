...
class CParNovaMetCom extends CParMain{
    var $city_id;
    static $name_parser = array(
        'novametcom' => 'Новаметалл Трейд- Москва и область '
    );
    public $items = array();
    function start(){
        ...
        foreach($this->document_list as $key => $path){
            if($key == 'metall'){
                ...
                $this->filter =  array(
                    ...
                    'coef' => 1,
                    'end' => '',
                    'header' => 'Сварная из проволоки ВР-1 в картах',
                    'skleyka' => false,
                    'dualheader'=>false,
                    'tree' => false,
                    'fixcell' => false,
                ...
                $this->filter =  array(
                    'propusk' => $this->num('BEGHI'),
                    'cost1' => 5,
                    'cost2' => 3,
                    'coef' => 1,
                    'end' => '',
                    'header' => 'Гладкий лист 1250',
                    'skleyka' => false,
                    'dualheader'=>false,
                    'tree' => false,
                    'fixcell' => false,
                ...
                $this->filter =  array(
                    ...
                    'dualheader'=>false,
                    'tree' => false,
                    'fixcell' => $this->num('ADE'),
                    'baseOfFixCell' => $this->num('CFF'),
    ...
    public function getUrl(){
        $parse = file_get_html('http://novametcom.ru/prajs/');
        $link = $parse->find('div.price a', 0);
        $href['metall'] = 'http://novametcom.ru'.$link->href;
        unset($link);
        $link = $parse->find('div.price_opt a', 0);
        $href['krep'] = 'http://novametcom.ru'.$link->href;
        unset($link);
        $link = $parse->find('div.price_lstk a', 0);
        $href['prof'] = 'http://novametcom.ru'.$link->href;
        return $href;
    }
}
