...
class CParSantehComplectUral extends CParMain{
    var $city_id;
    static $name_parser = array(
        'santehcomplecekb' => 'Сантехкомплект'
    );
    public $items = array();
    function start(){
    ...
    public function getUrl(){
        preg_match_all('~href=\W\S+.xls\W~', file_get_html('http://www.santur.ru/')->find('body',0)->innertext, $out);
        $links['main']='http://www.santur.ru'.$out[0][0];
        // p($out);
        //die();
        return str_replace(array('href=','"',"'"), '', $links);
    }
}
class CParSantehComplectTagil extends CParSantehComplectUral{
    function __construct(){
        parent::__construct();
        $this->price_id = 11484959;
        $this->document_name= 'SantehComplectTagil'.'_'.date('d-m-Y', time()).'_'.time().'.csv';
    }
}
class CParSantehComplectChelab extends CParSantehComplectUral{
    function __construct(){
        parent::__construct();
        $this->price_id = 11484960;
        $this->document_name= 'SantehComplectChelab'.'_'.date('d-m-Y', time()).'_'.time().'.csv';
    }
}
class CParSantehComplectTumen extends CParSantehComplectUral{
    function __construct(){
        parent::__construct();
        $this->price_id = 11484961;
        $this->document_name= 'SantehComplectTumen'.'_'.date('d-m-Y', time()).'_'.time().'.csv';
    }
}
class CParSantehComplectSurgut extends CParSantehComplectUral{
    function __construct(){
        parent::__construct();
        $this->price_id = 11484962;
        $this->document_name= 'SantehComplectSurgut'.'_'.date('d-m-Y', time()).'_'.time().'.csv';
    }
}