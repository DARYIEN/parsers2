...
class CParEVRASIjevsk extends CParMain{
    var $city_id;
    static $name_parser = array(
        'evrasijevsk' => 'ЕвразМеталл Ижевск'
    );
    function start(){
    ...
    function __construct(){
        ...
        $this->document_url = 'http://www.emural.ru/files/0/141396771390543401.xls';
        //$this->coef = 1000;
        $this->dual_cost = false;
        $this->price_id = 10148822;
    }
        ...
        foreach($this->sheet as $rows){
            ...
            foreach($rows as $num_row => $row){
                if(!empty($row)){
                    ...
                    if($num_row == $this->filter['horizontal']['from']['numeric'] ) $name =trim($header);
