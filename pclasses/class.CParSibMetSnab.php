...
class CParSibMetSnab extends CParMain{
    var $city_id;
    static $name_parser = array(
        'sibmetsnab' => 'СИБМЕТСНАБ'
    );
    function start(){
    ...
    function __construct(){
        ...
        $this->document_url = 'http://www.sibmetsnab.ru/images/M_images/Price/price-novosibirsk-10-01-2014.xlsx';
        //$this->coef = 1000;
        $this->dual_cost = true;
        $this->price_id = 8471109;
    }
        ...
        foreach($this->sheet as $rows){
            ...
            foreach($rows as $num_row => $row){
                if(!empty($row)){
                    ...
                    if(in_array($row, array('цены действуют при условии 100% п/п', 'Наименование'))){
                        continue;
                    }
                    if(count($rows) == 1 ){
                        $header = $row;
