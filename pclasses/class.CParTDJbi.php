...
class CParTDJbi extends CParMain{
    var $city_id;
    ...
    function __construct(){
        ...
        $this->document_urls['main'] = 'http://gbi5.ru/PRICE_TD_GBI-5.xls';
        ...
        foreach($this->document_list as $key => $path){
            if($key == 'main'){
                $this->filter =  array(
                    ...
                    'coef' => 1,
                    'end' => '35x35',
                    'header' => "Сваи железобетонные цельные",
                    'skleyka' => false,
                    'dualheader'=>false,
                    'tree' => false,
                    'fixcell' => false,
    ...
    function documentParsing(){
        ...
        foreach($this->sheet as $rows){
            ...
            if(!empty($name)&& !empty($costs)){
                $this->items[] = array('name' => str_replace(["º","É","÷","см²"],["","E","-","см2"],preg_replace('/Ǿ/iu','',preg_replace('/\?/iu','', preg_replace('/_/iu',' ', preg_replace('/;/iu','!', preg_replace('/\s+/iu',' ',trim($header.' '.$treeString.' '.$name.' '.$this->filter['end']))))))), 'cost' => $costs);
            }
            $name = '';
            $cost = '';
            unset($costs);
