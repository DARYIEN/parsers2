...
class CParMetKompMsk extends CParMain{
    var $city_id;
    static $name_parser = array(
        'metkomp' => 'Металлокомплект-М (МКМ-Москва)'
    );
    public $items = array();
    function start(){
    ...
    function __construct(){
        ...
        $this->document_urls['main'] = 'http://www.metkomp.ru/mkm-moscow/price/mkmmoscow-price.xls';
        ...
        foreach($this->sheet as $rows){
            ...
            foreach($rows as $num_row => $row){
                if(!empty($row)){
                    ...
                    if(in_array($row, array('1-3,5 тн'))){
                        continue 2;
                    }
