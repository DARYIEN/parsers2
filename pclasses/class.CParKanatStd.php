...
class CParKanatStd extends CParMain{
    var $city_id;
    static $name_parser = array(
        'kanatstdspb' => 'Канат СТД'
    );
    public $items = array();
    function start(){
    ...
    function __construct(){
        ...
        $this->document_urls['main'] = 'http://metizgroup.ru/wp-content/uploads/Kanat-Pricelist-SPB.xls';
            ...
            foreach($rows as $num_row => $row){
                if(!empty($row)){
                    ...
                    if(in_array($num_row, array($this->filter['horizontal']['to']['numeric'] - $this->filter['cost2']))){
                        ...
                        if(!is_numeric($cost) || is_null($cost)||$cost==1){
                            $name = '';
                            $cost = '';
                            continue 2;
                        }
...
class CParKanatStdMoscow extends CParKanatStd{
    function __construct(){
        parent::__construct();
        $this->document_urls['main'] = 'http://metizgroup.ru/wp-content/uploads/Kanat-Pricelist-MSK.xls';
        $this->price_id = 11506137;
        $this->document_name= 'KanatStdMoscow'.'_'.date('d-m-Y', time()).'_'.time().'.csv';
    }
}
class CParKanatStdKrasnodar extends CParKanatStd{
    function __construct(){
        parent::__construct();
        $this->document_urls['main'] = 'http://metizgroup.ru/wp-content/uploads/Kanat-Pricelist-KR.xls';
        $this->price_id = 11506140;
        $this->document_name= 'KanatStdKrasnodar'.'_'.date('d-m-Y', time()).'_'.time().'.csv';
    }
}