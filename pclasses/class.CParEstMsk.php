...
class CParEstMsk extends CParMain{
    var $city_id;
    ...
    function __construct(){
        ...
        $this->document_urls = $this->getUrl('http://www.est-msk.ru/?id=4');
            ...
            if($key == 'main'){
                ...
                for($i=74; $i<=88; $i++){
                    $this->filter =  array(
                        ...
                        'cost2' => 0,
                        'coef' => 1,
                        'begin' => 'трубы в ППУ в полиэтиленовой оболочке   ',
                        'end' => 'gaga',
                        'header' => '',
                        'skleyka' => false,
                        'dualheader'=>false,
                    ...
                    $this->filter =  array(
                        ...
                        'cost2' => 0,
                        'coef' => 1,
                        'begin' => 'трубы в ППУ в оцинкованной оболочке',
                        'end' => 'gaga',
                        'header' => '',
                        'skleyka' => false,
                        'dualheader'=>false,
                ...
                for($i=74; $i<=84; $i++){
                    ...
                    $this->filter =  array(
                        ...
                        'cost2' => 0,
                        'coef' => 1,
                        'begin' => 'Тройник паралельный в ППУ-ПЭ (ОЦ)',
                        'end' => 'gaga',
                        'header' => '',
                        'skleyka' => false,
                        'dualheader'=>false,
                    ...
                    $this->filter =  array(
                        ...
                        'cost2' => 0,
                        'coef' => 1,
                        'begin' => 'Тройниковое отвлетление в ППУ-ПЭ (ОЦ)',
                        'end' => 'gaga',
                        'header' => '',
                        'skleyka' => false,
                        'dualheader'=>false,
        ...
        foreach($this->sheet as $rows){
            ...
            foreach($rows as $num_row => $row){
                if(!empty($row)){
                    ...
                    if($this->filter['end'] == 'gaga') $this->filter['end'] = $row;
                    if($this->filter['begin'] == 'muhaha' && $num_row == $this->filter['horizontal']['to']['numeric']){
                        $this->filter['begin'] = $row;
                        continue;
                    }
                    ...
                    if(in_array(trim($row), array(
                        'ООО ТД «Армроскомплект» является  дилером ООО «ТД «Кичигинский» и предлагает гидранты пр-ва Кичигинского рем. Завода.С прайс-листом завода можно ознакомиться ниже (прайс№1)',
                        '32 х 3;  3,5;  4',
                        '38 х 3;  3,5;  4',
                        '42 х 3; 3,5; 4',
                        '45 х 3,5;  5',
                        '48,3 х 3,5   51 х 2,5',
                        'Ø трубы Ду (ГОСТ 3262)',
                        'стоимость трубы Ø  102 мм',
                        'стоимость трубы Ø  114 мм',
                        'для ОЦ оболочки Ø 100 мм',
                        'Цены указаны с НДС',
                    ))){
                        continue;
                    }
            ...
            if(!empty($name)&& !empty($costs)){
                $this->items[] = array('name' => preg_replace('/Ø/iu','',preg_replace('/Ǿ/iu','',preg_replace('/\?/iu','', preg_replace('/;/iu','!', preg_replace('/\s+/iu',' ',trim($this->filter['begin'].' '.$header.' '.$treeString.' '.$name.' '.$this->filter['end'])))))), 'cost' => $costs);
            }
            $name = '';
            $cost = '';
            unset($costs);
