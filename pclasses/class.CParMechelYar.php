...
class CParMechelYar extends CParMain{
    var $city_id;
    static $name_parser = array(
        'mechelyar' => 'Мечел-Сервис-Яр'
    );
    public $items = array();
    function start(){
    ...
    function __construct(){
        ...
        $this->document_urls = $this->getUrl('http://www.mechelservice.ru/contact.aspx?top_id=4&page_id=4');
    ...
    public function getUrl($url){
        foreach(file_get_html($url)->find('div.regionCity') as $div){
            if($div->find('div.regionCityName a',0)->name == "Ярославль"){
                foreach($div->find('a') as $link){
                    if(trim($link->plaintext) == "Скачать прайс-лист"){
                       $href = $link->href;
