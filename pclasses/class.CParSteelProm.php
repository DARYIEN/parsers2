...
class CParSteelProm extends CParMain{
    var $city_id;
    ...
    function __construct(){
        ...
        $this->document_url = 'http://www.stalprom.com/upload/prise.xls';
        //$this->coef = 1000;
        $this->dual_cost = true;
        $this->price_id = 10510613;
    }
        ...
        foreach($this->sheet as $rows){
           ...
           // p($rows);
            foreach($rows as $num_row => $row){
                if(!empty($row)){
                    if(in_array($num_row, array($this->filter['horizontal']['to']['numeric']))||in_array($num_row, array($this->filter['horizontal']['to']['numeric']-1))){
                    ...
                    if($num_row == $this->n('P') ||$num_row == $this->n('O') ||$num_row == 22 || $num_row == 20){
                        continue;
                    }
