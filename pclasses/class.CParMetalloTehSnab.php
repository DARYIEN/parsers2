...
class CParMetalloTehSnab extends CParMain{
    var $city_id;
    static $name_parser = array(
        'metallotehsnab' => 'Металлотехснаб'
    );
    function start(){
    ...
    function __construct(){
        ...
        $this->document_url = 'http://mtspr-1.ru/full.xls';
        //$this->coef = 1000;
        $this->dual_cost = true;
        $this->price_id = 10436099;
    }
        ...
        foreach($this->sheet as $rows){
            ...
            foreach($rows as $num_row => $row){
                if(!empty($row)){
                    ...
                    if(count($rows) == 1 ){
                        if(in_array(trim($row), array('80х80х6')))continue;
