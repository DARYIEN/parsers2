...
class CParTerritoriaStroyitelnogoKrepezha extends CParMain{
    var $city_id;
    static $name_parser = array(
        'territoriastroyitelnogokrepezha' => 'Территория Строительного Крепежа'
    );
    public $items = array();
    function start(){
    ...
    function __construct(){
        ...
        //$this->document_urls['main'] = 'http://armroskomplekt.ru/upload/price.xls';
        //$this->document_urls['prof'] = 'http://www.spk.ru/price/spk-krsn/_m_/10786/_p_/file';
        //$this->coef = 1000;
        $this->dual_cost = false;
        $this->decimal = true;
        $this->price_id = 12284499;
        $this->author = 'Михаил';
    }
        ...
        foreach($this->sheet as $rows){
            ...
            foreach($rows as $num_row => $row){
                if(!empty($row)){
                    ...
                    if(in_array(trim($row), array('11с67п 15 приварка'))){
                        $header='Кран шаровый стальной';
                    }
                    ...
                    if($this->filter['header']  && !is_numeric($rows[$this->filter['horizontal']['to']['numeric']])&& $this->filter['horizontal']['from']['numeric'] == $num_row){
    ...
    public function getUrl(){
        //preg_match_all('~href=\W\S+.xlsx{0,1}\W~', file_get_html('http://samorez74.ru/price/')->find('body',0)->innertext, $out);
        $parse = file_get_html('http://samorez74.ru/price/');
        $link = $parse->find('div.article p a', 0);
