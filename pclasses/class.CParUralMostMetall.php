...
class CParUralMostMetall extends CParMain{
    var $city_id;
    var $message = '';
    var $document_urls;
    var $home_url;
    ...
    static $name_parser = array(
        'uralmostmetall' => 'УралМост металл'
    );
    function start(){
        $this->getDocuments();
        $this->processParsing();
    ...
    function __construct(){
        ...
        $this->price_type = 'web';
        $this->dual_cost = true;
        $this->document_urls = array(   'http://xn----7sb7afkhmib.xn--p1ai/prays-list',
                                    );
        $this->home_url = current($this->document_urls);
        $this->price_id = 10660781;
        $this->iconv = true;
    }
    ...
    function documentParsing($path){
        ...
        $headers = $parse->find('table');
        $tables = $parse->find('table');
        if(!empty($headers)){
            foreach($headers as $header){
                foreach($header->find('td strong') as $content){
