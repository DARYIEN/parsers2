...
class CParMetalloSklad extends CParMain{
    var $city_id;
    static $name_parser = array(
        'metallosklad' => 'МеталлоСклад'
    );
    public $items = array();
    function start(){
    ...
    function __construct(){
        ...
        $this->document_url = 'http://www.metallosklad.kz/price/price.xls';
        //$this->coef = 1000;
        $this->dual_cost = true;
        $this->price_id = 8705146;
        $this->author = 'Феликс';
    }
        ...
        foreach($this->sheet as $rows){
            ...
            foreach($rows as $num_row => $row){
                if(!empty($row)){
                    ...
                    if($this->filter['header']&&$num_row == $this->filter['horizontal']['from']['numeric']+$this->filter['beginOfPrice'] ) $name =str_replace('ВГП, ЭСВ','',$header.' '.$treeString);
                    $new_header=false;
