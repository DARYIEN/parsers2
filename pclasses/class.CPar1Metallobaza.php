...
class CPar1Metallobaza extends CParMain{
    var $city_id;
    static $name_parser = array(
        '1metallobaza' => 'Первая Металлобаза'
    );
    function start(){
    ...
    function __construct(){
        ...
        //$this->document_url = 'http://www.1metallobaza.ru/sites/default/files/dir_files/freshprice_14.xls';
        //$this->coef = 1000;
        $this->document_url = $this->getUrl();
        $this->dual_cost = true;
        $this->price_id = 10101201;
    }
        ...
        foreach($this->sheet as $rows){
            ...
            foreach($rows as $num_row => $row){
                if(!empty($row)){
                    ...
                    if($num_row == 3||$num_row == 5){
                         continue;
                    }

                    if(in_array($row, array('цена за кг', ))){
                        $coef=1000;
                    }
    ...
    public function getUrl(){
        preg_match_all('~href=\W\S+.xlsx{0,1}\W~', file_get_html('http://www.1metallobaza.ru/price')->find('body',0)->innertext, $out);
        $links=$out[0][0];
        // p($out);
