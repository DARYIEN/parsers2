...
class CParNizegorodskyOpitnoExpZavod extends CParMain{
    var $city_id;
    static $name_parser = array(
        'nizegorodskyopitnoexpzavod' => 'Нижегородский опытно-экспериментальный завод - НОЭЗ'
    );
    public $items = array();
    function start(){
    ...
    function __construct(){
        ...
        $this->document_urls['main'] = 'http://www.noez52.ru/content/setup/pics/prices/priceNOEZ_all%202014.xls';
        //$this->coef = 1000;
        $this->dual_cost = false;
        $this->decimal = true;
        $this->price_id = 12267592;
        $this->author = 'Михаил';
    }
            ...
            foreach($rows as $num_row => $row){
                if(!empty($row)){
                    ...
                    if($this->filter['fixcell']!== false && in_array($num_row, $this->filter['fixcell'])){
                        $fixCell=$row;
                        if(in_array(trim($fixCell), array('желтый цинк'))){
                            $fixCell ='САМОРЕЗЫ УНИВЕРСАЛЬНЫЕ желтый цинк';
                        }
                        continue;
                    }
