...
class CParMiramall extends CParMain{
    var $city_id;
    ...
    function __construct(){
        ...
        $this->document_urls = $this->getUrl('https://miramall.ru/catalog/gipsokarton/');
	//$this->document_urls['main'] = "https://miramall.ru/miramall.ru_price.xls";
        ...
        foreach(file_get_html($url)->find('body a') as $link){
            ...
            if(strripos($href,'.xls' )!==false){
                if(strripos($href, parse_url($url, PHP_URL_HOST))!== true){
                    $href = 'https://'.parse_url($url, PHP_URL_HOST).'/'.$href;
                }
                $hrefs[] = $href;
            }
        }
