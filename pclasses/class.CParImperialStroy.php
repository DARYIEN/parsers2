...
class CParImperialStroy extends CParMain{
    var $city_id;
    var $message = '';
    var $document_urls;
    var $home_url;
    ...
    function __construct(){
        ...
        $this->document_urls = array(   'http://6699952.ru',
                                    );
        $this->home_url = current($this->document_urls);
        $this->price_id = 8883232;
        $this->iconv = true;
    }
    ...
    function documentParsing($path){
        ...
        $headers = $parse->find('table.table');
        $tables = $parse->find('table.table');
        if(!empty($headers)){
            foreach($headers as $header){
                foreach($header->find('h3') as $content){
                    $text[]=trim(str_replace(array('Прайс лист на','Прайс-лист на'), '', $content->innertext));
            ...
            //p($text);
               ...
               $cell=array(
                   'standart'=>array('stop_cell'=>4, 'cost_cell1'=>5,),
               );
                ...
                foreach($table->find('tr') as $tr){
                    ...
                    foreach($tr->find('td') as $td){
                        ...
                        if($step == 1){
                            ...
                            $cost = trim(strip_tags(str_replace(' ','',preg_replace('~[^0-9]+~','',$td->plaintext))));
                            //$cost = iconv('UTF-8','windows-1251',$cost);
                            $cost = str_replace(array(' руб.','50000'), '', $cost);
                            //$cost = str_replace(array('&nbsp;', '&#160;'), array('', ''), $cost);
                            //$cost = preg_replace('/\s+/iu','', $cost);
                        }else if ($step == 2){
                            $name = /*$text[$key].' '.*/$td->plaintext;
                   ...
                   // }
                    ...
                    $name = trim($name);
                    $name = str_replace(';','!', $name);
                    $name = str_replace(array('Диаметр трубы и толщина стенки:', 'Характеристики:', 'Описание:', ' б/у труба', 'состояние хорошее, труба для дальнейшего использования,', '', '', '', ''),'', $name);

                    $result[]=array('name'=>preg_replace('/\s+/iu',' ', 'Труба б/у '.$name),'cost'=>$cost,);
                }
...
class CParImperialStroyJelDor extends CParImperialStroy{
    function __construct(){
        parent::__construct();
        $this->price_id = 10688608;
    }
}