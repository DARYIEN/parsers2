...
class CParTorgstroymat extends CParMain{
    var $city_id;
    static $name_parser = array(
        'torgstroymat' => 'TorgStroyMat.ru'
    );
    public $items = array();
    function start(){
            ...
            foreach($rows as $num_row => $row){
                if(!empty($row)){
                    ...
                    if($this->filter['cost1']!==false && in_array($num_row, array($this->filter['horizontal']['to']['numeric'] - $this->filter['cost1']))){
                        $cost = Round(str_replace(array(' ','руб. / шт.','руб.',',','р.'),'',$row) * $this->filter['coef']);
