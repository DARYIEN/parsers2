...
class CParOmegaMetall extends CParMain{
    var $city_id;
    var $message = '';
    var $document_urls;
    var $home_url;
    ...
    function __construct(){
        ...
        $this->price_type = 'web';
        $this->dual_cost = true;
        $this->document_urls = array(   'http://www.omegametall.ru/pricelists/?action=show&id=8',
                                    );
        $this->home_url = current($this->document_urls);
        $this->price_id = 10093675;
        $this->iconv = true;
    }
    ...
    function documentParsing($path){
        ...
        $headers = $parse->find('table.price');
        $tables = $parse->find('table.price');
        if(!empty($headers)){
            foreach($headers as $header){
                foreach($header->find('td.p-c') as $content){
            ...
            //p($text);
               ...
               $cell=array(
                   'standart'=>array('stop_cell'=>3, 'cost_cell1'=>4, 'cost_cell2'=>6,),
               );
            ...
            foreach($tables as $key=>$table){
                ...
                foreach($table->find('tr') as $tr){
                    ...
                    foreach($tr->find('td') as $td){
                        ...
                        if(in_array($td->plaintext,$text)) $headerNum++;
                        if($step == 1){
                            //continue 2;
                        }else if ($step == 3){
                            $name = $text[$headerNum].' '.$td->plaintext;
                        }else if ($step >= 4 && $step <= $end_cell['stop_cell']){
                            $name .= ' '.$td->plaintext;
                        }else if ($step == $end_cell['cost_cell1']){
                            if (!empty($td->plaintext))
                                $cost[] = str_replace(' ', '',trim(strip_tags($td->plaintext)));
