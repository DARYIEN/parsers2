...
class CParMetalProekt extends CParMain{
    var $city_id;
    static $name_parser = array(
        'metalproekt' => 'МеталлПроект'
    );
    public $items = array();
    function start(){
    ...
    function __construct(){
        ...
        $this->document_urls['black'] = 'http://gk-proekt.ru/upload/price/cherniy_metalloprokat.xls';
        $this->document_urls['nerj'] = 'http://gk-proekt.ru/upload/price/nerzh_metalloprokat.xls';
        $this->document_urls['prof'] = 'http://gk-proekt.ru/upload/price/profnastil.xls';
        ...
        foreach($this->sheet as $rows){
            ...
            foreach($rows as $num_row => $row){
                if(!empty($row)){
                    ...
                    if($this->key =='black' && in_array($row, array('пог. м','шт.'))){
                        $name ='';
                        continue 2;
                    }
                    if($this->key =='prof' && in_array($row, array('пог. м','шт.'))){
                        $name ='';
                        continue 2;
                    }
