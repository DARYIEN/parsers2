...
class CParAstor extends CParMain{
    var $city_id;
    ...
    function __construct(){
        ...
        $this->document_url = 'http://www.steel.msk.ru/price/pricelst.zip';
        $this->file_list['price'] = ROOT.'/'.$this->dirArray['zip'].'/Astor.xls';
        //$this->coef = 1000;
        $this->dual_cost = true;
        $this->price_id = 10912383;
    }
        ...
        foreach($this->sheet as $rows){
            ...
            foreach($rows as $num_row => $row){
                if(!empty($row)){
                    ...
                    if($num_row == $this->filter['horizontal']['from']['numeric']+1){
                        if(stripos($row,'кг')!==false){
                            $this->filter['coef'] = 1000;
                        }
                        if(stripos($row,'шт')!==false || strripos($row,'м.кв')!==false){
                            $this->filter['coef'] = 1;
                        }
                    }
            ...
            if(!empty($name)&& !empty($costs)){
                $this->items[] = array('name' => str_replace(array('?',),'',preg_replace('/Ǿ/iu','',preg_replace('/\?/iu','', preg_replace('/;/iu','!', preg_replace('/\s+/iu',' ', $name.' '.$this->filter['end']))))), 'cost' => $costs);
            }
            $name = '';
            $cost = '';
            unset($costs);
