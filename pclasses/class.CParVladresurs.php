...
class CParVladresurs extends CParMain{
    var $city_id;
    static $name_parser = array(
        'vladresurs' => 'Владресурс'
    );
    public $items = array();
    function start(){
    ...
    function __construct(){
        ...
        $this->document_urls['main'] = 'http://vladresurs.ru/sites/default/files/metall_vladresurs.xlsx';
        //$this->coef = 1000;
        $this->dual_cost = false;
        $this->decimal = true;
        $this->price_id = 12267447;
        $this->author = 'Михаил';
    }
        ...
        foreach($this->sheet as $rows){
            ...
            foreach($rows as $num_row => $row){
                if(!empty($row)){
                    ...
                    if(in_array(trim($row), array('=======Осуществляем резку любого металла :прямоугольник сечением- 1200х1400,круга ф1200, весом до 30т, автоматическая высокоскоростная резка до ф510мм======='))){
                        continue;
                    }
