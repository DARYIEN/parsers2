...
class CParUniversalParser extends CParMain{
    var $city_id;
    ...
    function __construct(){
        ...
        $this->document_urls = $this->getUrl('regExp');
    ...
    public function getUrl($type/*, $options*/){
        switch($type){
            case'regExp':
                foreach(file_get_html('http://samorez74.ru/price/')->find('body a') as $link){
                    $hrefs[]=$link->href;
                }
                //mb_strri('~.xlsx{0,1}~', $hrefs, $out);
                //p($out);
