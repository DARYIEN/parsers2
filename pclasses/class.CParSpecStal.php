...
class CParSpecStal extends CParMain{
    var $city_id;
    static $name_parser = array(
        'specstal' => 'СпецСталь М'
    );
    public $items = array();
    function start(){
    ...
    function __construct(){
        ...
        $this->document_urls['main'] = 'http://www.specstal.com/images/downloads/price.xls';
        //$this->document_urls['prof'] = 'http://www.spk.ru/price/spk-krsn/_m_/10786/_p_/file';
        //$this->coef = 1000;
        $this->dual_cost = false;
        $this->decimal = true;
        $this->price_id = 12284473;
        $this->author = 'Михаил';
    }
            ...
            foreach($rows as $num_row => $row){
                if(!empty($row)){
                    ...
                    if($this->filter['header']  && count($rows) == 1 && $this->filter['horizontal']['from']['numeric'] == $num_row){
                        ...
                        if(in_array(trim($row), array('Горячекатаные листы марок стали 10ХСНД и 15ХСНД в наличии'))){
                            $header='Горячекатаные листы';
                        }
                        if(in_array(trim($row), array('Горячекатаные листы марки стали 09Г2С в наличии'))){
                            $header='Горячекатаные листы';
                        }
                        if(in_array(trim($row), array('КРУГИ марок стали 09Г2С и 20Х13 в наличии'))){
                            $header='КРУГИ';
                        }
                        $new_header=true;
                        continue;
