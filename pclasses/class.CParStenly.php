...
class CParStenly extends CParMain{
    var $city_id;
    ...
    function __construct(){
        ...
        $this->document_url = 'http://stanly.ru/files/File/price-stanly.zip';
    ...
    function documentParsing(){
        ...
        foreach($this->sheet as $rows){
            ...
            if(!empty($name)&& !empty($costs)){
                $this->items[] = array('name' => preg_replace('/"/iu','',preg_replace('/′/iu','',preg_replace('/Ǿ/iu','',preg_replace('/\?/iu','', preg_replace('/;/iu','!', preg_replace('/\s+/iu',' ',trim(iconv('CP1251','UTF-8',iconv('UTF-8','CP1252',$header.' '.$treeString.' '.$name.' '.$this->filter['end']))))))))), 'cost' => $costs);
            }
            $name = '';
            $cost = '';
            unset($costs);
