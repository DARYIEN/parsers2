 ...
 * Time: 12:44
 */
class CParNewKtz extends CParMain{
    var $city_id;
    var $message = '';
    var $document_urls;
    var $home_url;
    ...
    static $name_parser = array(
        'ktznew' => 'Королевский трубный завод'
    );
    function start(){
        $this->getDocuments();
        $this->processParsing();
    ...
    function __construct(){
        ...
        $this->company_name = current(self::$name_parser).' '.current(array_values($this->cities_list[$this->city_id]));
        $this->formDirsArray()->createDirs();
        $this->document_extended = '.html';
        $this->home_url = 'http://www.ktzholding.com/product/show/1';
    ...
    function documentParsing($path){
        ...
        $table = $parse->find('table.index-prod-list tbody tr');
        if(!empty($table)){
            $result = array();
            foreach($table as $tr){
                $temprorary = array();
                ...
                foreach($tr->find('td') as $td){
                    ...
                    $temprorary[] =  trim(str_replace('&nbsp;', ' ', $td->innertext));
                }
                $temp['cost'] = array_pop($temprorary);
                $temp['name'] = $temprorary;
    ...
    public function getUrl(){
        ...
        $info_all = $parse->find('ul.left-menu li');
        $href = array();
        foreach($info_all as $info_first){
            $info_second = $info_first->find('a',0);
            $href[] = 'http://www.ktzholding.com'.$info_second->href;
            //break;
        }
...
class CParNewKtzYaros extends CParNewKtz{
  function __construct(){
