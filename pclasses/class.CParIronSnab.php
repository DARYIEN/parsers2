...
class CParIronSnab extends CParMain{
    var $city_id;
    var $message = '';
    var $document_urls;
    var $home_url;
    ...
    function __construct(){
        ...
        $this->document_urls = array(   'http://www.iron-snab.ru/price/',
                                    );
        $this->home_url = current($this->document_urls);
        $this->price_id = 9079929;
        $this->iconv = true;
    }
    ...
    function documentParsing($path){
        ...
        $headers = $parse->find('div.moduletable');
        $tables = $parse->find('table.priceTable');
        if(!empty($headers)){
            ...
            //p($text);
               ...
               $cell=array(
                   'standart'=>array('stop_cell'=>5, 'cost_cell1'=>4, 'cost_cell2'=>5,),
               );
            ...
            foreach($tables as $key=>$table){
                ...
                foreach($table->find('tr') as $tr){
                   ...
                   // }
                    ...
                    $result[]=array('name'=>preg_replace('/\s+/iu',' ', $name),'cost'=>$cost,);
                }
