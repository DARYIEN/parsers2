...
class CParUralTehProm extends CParMain{
    var $city_id;
    public $file_list = array();
    static $name_parser = array(
        'uraltehprom' => 'УралТехПром'
    );
    public $items = array();
    function start(){
    ...
    function __construct(){
        ...
        //$this->document_url = 'http://www.santehas.ru/downloads/price/santehas_price.zip';
    ...
    function documentParsing(){
        ...
        foreach($this->sheet as $rows){
            ...
            if(!empty($name)&& !empty($costs)){
                $this->items[] = array('name' => preg_replace('/"/iu','',preg_replace('/′/iu','',preg_replace('/Ǿ/iu','',preg_replace('/\?/iu','', preg_replace('/;/iu','!', preg_replace('/\s+/iu',' ',trim($header.' '.$treeString.' '.$name.' '.$this->filter['end']))))))), 'cost' => $costs);
            }
            $name = '';
            $cost = '';
            unset($costs);
    ...
    public function getUrl(){
        preg_match_all('~href=\W\S+.zip?\W~', file_get_html('http://utpchel.ru/')->find('body',0)->innertext, $out);
        $links='http://utpchel.ru'.$out[0][0];
        //p($out);
