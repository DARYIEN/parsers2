...
class CParBvbAlience extends CParMain{
    var $city_id;
    static $name_parser = array(
        'bvbalience_eburg' => 'BVB альянс Екатеринбург'
    );
    public $items = array();
    ...
    public function getUrl(){
        $parse = file_get_html('http://www.bvb-alyans.ru/');
        if($parse)$link = $parse->find('ul.myul2 li input.mybutton', 2);
        preg_match("/'.*'/", $link->onclick, $match);
        $href = str_replace("'",'',$match[0]);
        return 'http://www.bvb-alyans.ru'.$href;
    }
}
class CParBvbAlienceSurgut extends CParBvbAlience{
    function __construct(){
        parent::$name_parser = array(
            'bvbalience_surgut' => 'BVB Альянс Сургут '
        );
        parent::__construct();
        $this->price_id = 10958314;
    }
}
class CParBvbAlienceTumen extends CParBvbAlience{
    function __construct(){
        parent::$name_parser = array(
            'bvbalience_tumen' => 'BVB Альянс Сургут '
        );
        parent::__construct();
        $this->price_id = 10953882;
    }
}
class CParBvbAlienceUfa extends CParBvbAlience{
    function __construct(){
        parent::$name_parser = array(
            'bvbalience_ufa' => 'BVB Альянс Уфа '
        );
        parent::__construct();
        $this->price_id = 12274772;
    }
}