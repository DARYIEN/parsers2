...
class CParMetInvestGrup extends CParMain{
    var $city_id;
    static $name_parser = array(
        'metinvestgrup' => 'МетИнвестГрупп'
    );
    public $items = array();
    function start(){
    ...
    function __construct(){
        ...
        $this->document_urls['black'] = 'http://met-invest-group.ru/images/price/price.xls';
        //$this->coef = 1000;
        $this->dual_cost = true;
        $this->price_id = 11506127;
    }
            ...
            foreach($rows as $num_row => $row){
                if(!empty($row)){
                    ...
                    if(in_array($num_row, array($this->filter['horizontal']['to']['numeric'] - $this->filter['cost2']))){
                        $cost = Round(str_replace(array(' ','руб.','р/тн'),'',$row) * $this->filter['coef']);
                        if(!is_numeric($cost) || is_null($cost)){
                            $name = '';
                            $cost = '';
                            continue 2;
                    ...
                    if($num_row == $this->filter['horizontal']['from']['numeric'] && trim($rows[$this->filter['horizontal']['to']['numeric']])=='Цена' ){
