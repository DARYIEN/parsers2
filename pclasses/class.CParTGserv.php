 ...
 * Time: 12:44
 */
class CParTGserv extends CParMain{
    var $city_id;
    var $message = '';
    var $document_urls;
    var $home_url;
    ...
    function __construct(){
        ...
        $this->home_url = 'http://tgserv.ru/sklad-prajs.html';
    ...
    function documentParsing($path){
        ...
        $tables = $parse->find('table.table-hover');
        if(!empty($tables)){
            $result = array();
            foreach($tables as $table){
                $oneTable = $table->find("tbody tr");
                foreach($oneTable as $tr){
                    $temprorary = array();
                    $temp = array();
                    foreach($tr->find('td[style="text-align: right;"]') as $span){
                        //p($span->innertext);
                        //echo iconv('cp1251','utf-8',$td->innertext);
                        $temp['cost'] =  trim(str_replace(array("(т.)"," ","(шт.)","(пг.м.)","&nbsp;"),"",preg_replace('/'.chr(160).'/', '', $span->plaintext)));
                       // p($temp['cost']);
                    }
                    $i = 0;
                    if(!empty($temp) && !is_numeric($temp['cost'])){
                        //p($temp['cost']);
                        continue;
                    }
                    $partsOfName = $tr->find('td');
                    foreach($partsOfName as $td){
                        //p($td->innertext);
                        //echo iconv('cp1251','utf-8',$td->innertext);
                        $partOfName = $td->innertext;
                        $i++;
                        if($i == 1 || $i == count($partsOfName)) continue;
                        $temp['name'][] =  trim(str_replace('&nbsp;', ' ', $partOfName));
