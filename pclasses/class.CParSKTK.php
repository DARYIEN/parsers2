...
class CParCParSKTK extends CParMain{
    var $city_id;
    var $message = '';
    var $document_urls;
    var $home_url;
    ...
    function __construct(){
        ...
        $this->document_urls = array(   'http://www.sktk-ppu.ru/production/price_list/ppu/',
                                        'http://www.sktk-ppu.ru/production/price_list/ppm/',
                                        'http://www.sktk-ppu.ru/production/price_list/vus_tspi/',
                                        'http://www.sktk-ppu.ru/production/price_list/metall/',
                                    );
        $this->home_url = current($this->document_urls);
        $this->price_id = 8161648;
        $this->iconv = true;
    }
    ...
    function documentParsing($path){
        ...
        $headers = $parse->find('div.order_section');
        $tables = $parse->find('table');
        if(!empty($headers)){
            foreach($headers as $header){
                foreach($header->find('a.order_lnk') as $content){
            ...
            if(count($text)==18){
               $cell=array(
                   'standart'=>array('stop_cell'=>6, 'cost_cell'=>7,),
                   '6'=>array('stop_cell'=>6, 'cost_cell'=>10,),
                   '9'=>array('stop_cell'=>5, 'cost_cell'=>6,),
               );
            } else if(count($text)==4){
                $cell=array(
                    'standart'=>array('stop_cell'=>6, 'cost_cell'=>7,),
                );
                ...
                $cell=array(
                    'standart'=>array('stop_cell'=>6, 'cost_cell'=>7,),
                    '1'=>array('stop_cell'=>5, 'cost_cell'=>6,),
                    '2'=>array('stop_cell'=>5, 'cost_cell'=>6,),
                    '3'=>array('stop_cell'=>5, 'cost_cell'=>6,),
                    '8'=>array('stop_cell'=>5, 'cost_cell'=>6,),
                    '9'=>array('stop_cell'=>5, 'cost_cell'=>6,),
                    '10'=>array('stop_cell'=>4, 'cost_cell'=>5,),
                );
            }
        if(!empty($tables)){
            $result = array();
            ...
            foreach($tables as $key=>$table){
                $step = 1;
                if(in_array($table_number ,array(14,15,16,17))) continue ;
                foreach($table->find('tr') as $tr){
                    ...
                    if(count($text)==10 && $table_number == 10) $firstStep = 3; else $firstStep = 3;
                    ...
                    foreach($tr->find('td') as $td){
                        ...
                        }else if ($step >= $firstStep+1 && $step <= $end_cell['stop_cell']){
                            $name .= ' '.$td->innertext;
                        }else if ($step == $end_cell['cost_cell']){
                    ...
                    $step = 3;
                    if (empty($cost)|| !is_numeric($cost)) continue;
                    //$name .= ' '.$td->innertext;
