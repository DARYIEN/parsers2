...
class CParMetalloprokat extends CParMain{
    var $city_id;
    ...
    function __construct(){
        ...
        $this->document_urls['main'] = 'http://metall-pr.ru/price/kach.xls';
        $this->document_urls['truba'] = 'http://metall-pr.ru/price/trubi.xls';
        $this->document_urls['sort'] = 'http://metall-pr.ru/price/sort.xls';
        $this->document_urls['list'] = 'http://metall-pr.ru/price/listpr.xls';
        $this->document_urls['cvet'] = 'http://metall-pr.ru/price/tsvet.xls';
        $this->document_urls['nerzh'] = 'http://metall-pr.ru/price/nerzh.xls';
        ...
        foreach($this->document_list as $key => $path){
            if($key == 'main'){
	            ...
	            $this->filter =  array(
		            ...
		            'coef' => 1,
		            'end' => '',
		            'header' => "СТАЛЬ КОНСТРУКЦИОННАЯ  НИКЕЛ КРУГ",
		            'skleyka' => true,
		            'dualheader'=>false,
		            'tree' => false,
		            'fixcell' => false,
	            ...
	            $this->filter =  array(
		            ...
		            'coef' => 1,
		            'end' => '',
		            'header' => "ТРУБЫ ЭЛЕКТРОСВАРНЫЕ ПРЯМОУГ",
		            'skleyka' => true,
		            'dualheader'=>false,
		            'tree' => false,
		            'fixcell' => false,
	            ...
	            $this->filter =  array(
		            ...
		            'coef' => 1,
		            'end' => '',
		            'header' => "СТАЛЬ СОРТ НЕРЖ НИКЕЛ КРУГ",
		            'skleyka' => true,
		            'dualheader'=>false,
		            'tree' => false,
		            'fixcell' => false,
    ...
    function documentParsing(){
        ...
        foreach($this->sheet as $rows){
			...
			if(count($rows) > 1 && $glueString != ''){
				$this->items[count($this->items)-1]['name'] .=  preg_replace("/×/","Х",preg_replace('/Ǿ/iu','',preg_replace('/\?/iu','', preg_replace('/_/iu',' ', preg_replace('/;/iu','!', preg_replace('/\s+/iu',' ',trim(' '.$glueString)))))));
				//echo $rows[$this->filter['horizontal']['from']['numeric']];
				$glueString = '';
			}
	        array_reverse($rows);
            foreach($rows as $num_row => $row){
                if(!empty($row)){
	                if(empty($rows[$this->filter['horizontal']['from']['numeric']]) && $this->filter['glue'] !== false && in_array($num_row, $this->filter['glue'])){
		                $glueString .= '! '.$row;
		                continue;
	                }
            ...
            if(!empty($name)&& !empty($costs)){
                $this->items[] = array('name' => preg_replace('/\s+/iu',' ',preg_replace("/×/","Х",preg_replace('/Ǿ/iu','',preg_replace('/\?/iu','', preg_replace('/_/iu',' ', preg_replace('/;/iu','!', trim(str_replace("(продолжение)","",$header).' '.$treeString.' '.$name.' '.$this->filter['end']))))))), 'cost' => $costs);
            }
