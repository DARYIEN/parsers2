...
class CParStalInteh extends CParMain{
    var $city_id;
    static $name_parser = array(
        'stalinteh' => 'СтальИнтех'
    );
    public $items = array();
    function start(){
    ...
    function __construct(){
        ...
        $this->document_urls['main'] = 'http://steelintech.ru/assets/files/price.xls';
        //$this->coef = 1000;
        $this->dual_cost = false;
        $this->decimal = true;
        $this->price_id = 12243651;
        $this->author = 'Михаил';
    }
        ...
        foreach($this->sheet as $rows){
            ...
            foreach($rows as $num_row => $row){
                if(!empty($row)){
                    ...
                    if($this->filter['header']  && trim($rows[$num_row+2]) == 'Цена' && $this->filter['horizontal']['from']['numeric'] == $num_row){
