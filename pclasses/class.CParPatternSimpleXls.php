...
class CParPatternSimpleXls extends CParMain{
    var $city_id;
    static $name_parser = array(
        'troykastal' => 'ТройкаСталь'
    );
    function start(){
    ...
    function __construct(){
        ...
        $this->document_url = 'http://www.troykastal.ru/price.xls';
        //$this->coef = 1000;
        $this->dual_cost = true;
    ...
    function documentParsing(){
        ...
        foreach($this->sheet as $rows){
            ...
            foreach($rows as $num_row => $row){
                if(!empty($row)){
                    if(in_array($num_row, array($this->filter['horizontal']['to']['numeric'],$this->filter['horizontal']['to']['numeric'] - 1))){
                        ...
                        if(!is_numeric(end($cost)) || is_null($cost)){
                            $name = '';
                            $cost = '';
                            continue 2;
                        }
            ...
            if(!empty($name)&& !empty($cost)){
                $this->items[] = array('name' => preg_replace('/;/i',',',$name), 'cost' => $cost);
            }
            $name = '';
            $cost = array();
        }
