...
class CParStalnayaKompaniya extends CParMain{
    var $city_id;
    static $name_parser = array(
        'stalnayakompaniya' => 'Стальная Компания'
    );
    public $items = array();
    function start(){
           ...
           // p($rows);
            foreach($rows as $num_row => $row){
                if(!empty($row)){
                    ...
                    if(in_array(trim($row), array('Наименование'))){
                        continue;
                    }
                    ...
                    if(in_array($num_row, array($this->filter['horizontal']['to']['numeric'] - $this->filter['cost2']))){
                        ...
                        if(!is_numeric($cost) || is_null($cost)||$cost==9){
                            $name = '';
                            $cost = '';
                            continue 2;
                        }
    ...
    public function getUrl(){
        preg_match_all('~href=\W\S+.xlsx{0,1}\W~', file_get_html('http://stalcompany.com/page/prais-list/')->find('body',0)->innertext, $out);
        $links['main']='http://stalcompany.com'.$out[0][0];
        //p($out);
        //die();
