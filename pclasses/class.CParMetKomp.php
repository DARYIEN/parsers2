...
class CParMetKomp extends CParMain{
    var $city_id;
    static $name_parser = array(
        'MetKomp' => 'МетКомплектМ'
    );
    public $items = array();
        ...
        foreach($this->document_list as $key => $path){
            if($key == 'main'){
                ...
                $this->filter =  array(
                    ...
                    'coef' => 1,
                    'end' => '',
                    'header' => 'Сетка сварная для ЖБК ТУ 127600 ',
                    'skleyka' => false,
                    'dualheader'=>false,
                    'tree' => false,
                    'fixcell1' => $this->num('I'),
        ...
        foreach($this->sheet as $rows){
            ...
            foreach($rows as $num_row => $row){
                if(!empty($row)){
                    ...
                    if($this->filter['fixcell1']!== false && in_array($num_row, $this->filter['fixcell1'])){
                        $fixCell1=$row;
                        continue;
                    }
                    if($this->filter['fixcell1']!== false && in_array($num_row-1, $this->filter['fixcell1'])){
                        $name .= ' '.$fixCell1;
                    }
                    if($this->filter['fixcell2']!== false && in_array($num_row, $this->filter['fixcell2'])){
                        $fixCell2=$row;
                        continue;
                    }
                    if($this->filter['fixcell2']!== false && in_array($num_row-1, $this->filter['fixcell2'])){
                        $name .= ' '.$fixCell2;
                    }
                    if($this->filter['fixcell3']!== false && in_array($num_row, $this->filter['fixcell3'])){
                        $fixCell3=$row;
                        continue;
                    }
                    if($this->filter['fixcell3']!== false && in_array($num_row-1, $this->filter['fixcell3'])){
                        $name .= ' '.$fixCell2.' '.$fixCell3;
                    }
    ...
    public function getUrl(){
        $parse = file_get_html('http://www.regcm.ru/');
        $link = $parse->find('div.items div.item div.title a', 0);
        return $link->href;
    }
}