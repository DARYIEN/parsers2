...
class CParSteelMax extends CParMain{
    var $city_id;
    ...
    function __construct(){
        ...
        $this->document_urls['main'] = 'http://steel-max.ru/images/stories/prices/price_black.xls';
        $this->document_urls['cvet'] = 'http://steel-max.ru/images/stories/prices/price.xls';
        $this->document_urls['nerj'] = 'http://steel-max.ru/images/stories/prices/price_nerzh.xls';
        ...
        foreach($this->document_list as $key => $path){
            if($key == 'main'){
                ...
                $this->filter =  array(
                    ...
                    'coef' => 1000,
                    'end' => '',
                    'header' => 'Лист AISI 321 (08-12Х18Н10Т)',
                    'skleyka' => false,
                    'dualheader'=>false,
                    'tree' => false,
                    'fixcell' => false,
            ...
            foreach($rows as $num_row => $row){
                if(!empty($row)){
                    ...
                    if($this->filter['cost1']!==false && in_array($num_row, array($this->filter['horizontal']['to']['numeric'] - $this->filter['cost1']))){
                        $cost = str_replace(array(' ','от','руб.',',','р.','&#160;'),'',$row) * $this->filter['coef'];
                    ...
                    if(in_array($num_row, array($this->filter['horizontal']['to']['numeric'] - $this->filter['cost2']))){
                        ...
                        $cost = str_replace(array(' ','от','руб.','р.','&#160;','&nbsp;', chr(160),),'',$row) * $this->filter['coef'];
                        if(!is_numeric($cost) || is_null($cost)){
                            $name = '';
                            $cost = '';
                            continue 2;
