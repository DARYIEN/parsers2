...
class CParMetProfil extends CParMain{
    var $city_id;
    static $name_parser = array(
        'metProfil' => 'МетПрофиль'
    );
    public $items = array();
    function start(){
    ...
    function __construct(){
        ...
        $this->document_urls = $this->getUrl('http://metprofil.ru/');
    ...
    function processParsing(){
        foreach($this->document_list as $key => $path){
            if($key == 'main'){
                $this->filter =  array(
                    'propusk' => $this->num('DFGH'),
                    'cost1' => 4,
                    'cost2' => 0,
                    'coef' => 1,
                    'begin' => 'ПРОФНАСТИЛ ОЦИНКОВАННЫЙ',
                    'end' => '',
                    'header' => false,
                    'skleyka' => false,
                    'dualheader'=>false,
                ...
                $this->filter =  array(
                    ...
                    'cost2' => 0,
                    'coef' => 1,
                    'begin' => 'ПРОФНАСТИЛ ОЦИНКОВАННЫЙ С ПОЛИМЕРНЫМ ПОКРЫТИЕМ ПОЛИЭСТЕР (RAL3005;6005;8017;5005;7004;9003;9006 идр.)',
                    'end' => '',
                    'header' => false,
                    'skleyka' => false,
                    'dualheader'=>false,
