...
class CParDiposSpb extends CParMain{
    var $city_id;
    static $name_parser = array(
        'diposspb' => ' ДиПОС Санкт-Петербург'
    );
    function start(){
    ...
    function __construct(){
        ...
        $this->document_url = 'http://www.spb.dipos.ru/upd_prase/price.xls';
        //$this->coef = 1000;
        $this->dual_cost = false;
        $this->price_id = 8438130;
    }
        ...
        foreach($this->sheet as $rows){
            ...
            foreach($rows as $num_row => $row){
                if(!empty($row)){
                    ...
                    if(count($rows)==1||trim($row)=='АРМАТУРА класса А1'||trim($row)=='БАЛКА Ст3СП/ПС'||trim($row)=='ШВЕЛЛЕР ГНУТЫЙ  Ст3'||trim($row)=='ТРУБА профильная'||trim($row)=='ТРУБА ВГП ГОСТ 3262-75'||trim($row)=='СЕТКА из Вр-1'||trim($row)=='СЕТКА АРМАТУРНАЯ'||trim($row)=='Профнастил оцинков.'||trim($row)=='ЭЛЕКТРОДЫ сварочные'){
                        $header = $row;
                        continue;
                    }
