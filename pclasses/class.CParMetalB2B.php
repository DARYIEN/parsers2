...
class CParMetalB2B extends CParMain{
    var $city_id;
    static $name_parser = array(
        'MetalB2B' => 'ТД Феррум Москва'
    );
    public $items = array();
    function start(){
    ...
    function __construct(){
        ...
        $this->document_urls['main'] = 'http://www.metal-b2b.ru/yml_products.xml';
        //$this->coef = 1000;
        $this->dual_cost = false;
        $this->decimal = true;
        $this->price_id = 14739957;
        $this->author = 'Михаил';
    }
    ...
    function processParsing(){
        foreach($this->document_list as $key => $path){
            foreach(simplexml_load_file($path)->shop->offers->offer as $offer){
                $this->items[]=array('name'=>preg_replace('/Ǿ/iu','',preg_replace('/\?/iu','', preg_replace('/;/iu','!', preg_replace('/\s+/iu',' ',trim((string)$offer->name))))),'cost'=>array((string)$offer->price));
            }
        }
        $this->save();
    }
