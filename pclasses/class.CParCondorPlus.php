...
class CParCondorPlus extends CParMain{
    var $city_id;
    static $name_parser = array(
        'condorplus' => 'Кондор Плюс'
    );
    public $items = array();
    function start(){
    ...
    function __construct(){
        ...
        $this->document_url = 'http://condorplus.ru/xls/xls/17.03.2014.xls';
        //$this->coef = 1000;
        $this->dual_cost = true;
        $this->price_id = 11096917;
        $this->author = 'Феликс';
    }
    ...
    function documentParsing(){
        ...
        foreach($this->sheet as $rows){
            ...
            foreach($rows as $num_row => $row){
                if(!empty($row)){
                    if(in_array($rows[$this->filter['number_col_coef']], array('кг'))){
