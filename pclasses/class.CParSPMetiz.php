...
class CParSPMetiz extends CParMain{
    var $city_id;
    ...
    function __construct(){
        ...
        //$this->document_urls['main'] = 'http://www.gipsoplita.ru/price/price_stroymaterialy.xls';
        ...
        foreach($this->sheet as $rows){
            ...
            foreach($rows as $num_row => $row){
                if(!empty($row)){
                    ...
                    if(strripos($rows[$this->filter['horizontal']['from']['numeric']+1],'Цена')!== false && $this->filter['horizontal']['from']['numeric'] == $num_row){
    ...
    public function getUrl(){
                $parse = file_get_html('http://sp-metiz.ru/product/gauze/');
                $links['main'] = 'http://sp-metiz.ru'.$parse->find('ul.mainnav li a', 6)->href;
                return $links;
    }
}