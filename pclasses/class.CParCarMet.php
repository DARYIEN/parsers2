...
class CParCarMet extends CParMain{
    var $city_id;
        ...
        foreach($this->document_list as $key => $path){
            if($key == 'metiz'){
                $this->filter =  array(
                    ...
                    'coef' => 1,
                    'end' => '',
                    'header' => 'Арматура А-III, А500С, А500СП(ГОСТ 5781-82,СТО АСЧМ 7-93)',
                    'skleyka' => false,
                    'dualheader'=>false,
                    'tree' => false,
                    'horizontal' => array(
...
class CParCarMetJelDor extends CParCarMet{
    function __construct(){
        parent::__construct();
        $this->price_id = 11157242;
        $this->document_name= 'CarMetJelDor'.'_'.date('d-m-Y', time()).'_'.time().'.csv';
    }
}