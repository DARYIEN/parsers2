...
class CParMetalloff extends CParMain{
    var $city_id;
    ...
    function __construct(){
        ...
        $this->document_urls['main'] = 'http://www.metallof.ru/price/metalloprokat.xls';
        $this->document_urls['nerjaveika'] = 'http://www.metallof.ru/price/nerjaveika.xls';
        $this->document_urls['specstal'] = 'http://www.metallof.ru/price/specstal.xls';
        ...
        foreach($this->document_list as $key => $path){
            if($key == 'main'){
                ...
                $this->filter =  array(
                    ...
                    'cost2' => 0,
                    'coef' => 1,
                    'end' => 'Сетка сварная оцинкованная отделочная',
                    'header' => false,
                    'skleyka' => false,
                    'dualheader'=>false,
                    'tree' => false,
        ...
        foreach($this->sheet as $rows){
            ...
            foreach($rows as $num_row => $row){
                if(!empty($row)){
                    ...
                    if(in_array($row, array('Отводы нержавеющие AISI 304'))){
                        $this->filter['coef'] = 1;
                    }
                    if(in_array($row, array('Круг нержавеющий'))){
                        $this->filter['coef'] = 1000;
                    }
                    if(in_array($row, array('Позиции под заказ'))){
                        continue;
                    }
            ...
            if(!empty($name)&& !empty($costs)){
                $this->items[] = array('name' => preg_replace('/ост./iu','',preg_replace('/имп./iu','',preg_replace('/"/iu','',preg_replace('/Ǿ/iu','',preg_replace('/\?/iu','', preg_replace('/;/iu','!', preg_replace('/\s+/iu',' ',trim($header.' '.$treeString.' '.$name.' '.$this->filter['end'])))))))), 'cost' => $costs);
            }
            $name = '';
            $cost = '';
            unset($costs);
    ...
    public function getUrl(){
        preg_match_all('~href=\W\S+.xlsx{0,1}\W~', file_get_html('http://www.sevmetallsnab.ru/')->find('body',0)->innertext, $out);
        $links['main']='http://www.sevmetallsnab.ru'.$out[0][0];
