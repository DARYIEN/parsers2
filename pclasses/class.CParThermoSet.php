...
class CParThermoSet extends CParMain{
    var $city_id;
    var $message = '';
    var $document_urls;
    var $home_url;
    ...
    function __construct(){
        ...
        $this->dual_cost = true;
        $this->author = 'Михаил';
        $this->document_urls = array(   'http://thermoset.ru/pricelist/',
                                    );
    ...
    function documentParsing($path){
        ...
        $tables = $parse->find('div.cpt_maincontent center table');
//        if(!empty($headers)){
                ...
                foreach($table->find('tr') as $tr){
                    ...
                    foreach($tr->find('td') as $key =>  $td){
                        if(count($tr->find('td')) > 1 && $key == count($tr->find('td')) - 1) {
                            $cost = str_replace(array(' ', ',', 'Р', '&#160;', '&nbsp;', chr(160), 'От', 'от'), '', $td->plaintext);
