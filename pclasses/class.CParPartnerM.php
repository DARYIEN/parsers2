...
class CParPartnerM extends CParMain{
    var $city_id;
    ...
    function __construct(){
        ...
        $this->document_url = 'http://www.partnerm.ru/price.xls';
        //$this->coef = 1000;
        $this->dual_cost = true;
        $this->price_id = 10660785;
    }
            ...
            foreach($rows as $num_row => $row){
                if(!empty($row)){
                    if(in_array($num_row, array($this->filter['horizontal']['to']['numeric']))){
                        $cost = Round( str_replace(' ','',$row)*$this->filter['coef']);
                    ...
                    if(in_array($num_row, array($this->filter['horizontal']['to']['numeric']-$this->filter['dermolist']))){
