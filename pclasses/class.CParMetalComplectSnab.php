...
class CParMetalComplectSnab extends CParMain{
    var $city_id;
    static $name_parser = array(
        'metalcomplectsnab' => 'МеталлКомплектСнаб'
    );
    function start(){
    ...
    function __construct(){
        ...
        $this->document_url = 'http://www.mkc54.ru/price/price.xls';
        //$this->coef = 1000;
        $this->dual_cost = true;
        $this->price_id = 10510623;
    }
    ...
    function documentParsing(){
        ...
        foreach($this->sheet as $rows){
            ...
            foreach($rows as $num_row => $row){
                if(!empty($row)){
                    if(in_array($num_row, array($this->filter['horizontal']['to']['numeric']))||in_array($num_row, array($this->filter['horizontal']['to']['numeric']-1))||in_array($num_row, array($this->filter['horizontal']['to']['numeric']-2))){
            ...
            if(!empty($name)&& !empty($cost)){
                $this->items[] = array('name' => preg_replace('/\?/iu','', preg_replace('/;/iu',',', preg_replace('/\s+/iu',' ', $name))), 'cost' => $arrCost);
            }
            $name = '';
            $cost = '';
            unset($arrCost);
