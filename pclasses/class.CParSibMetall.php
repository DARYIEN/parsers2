...
class CParSibMetall extends CParMain{
    var $city_id;
    ...
    function __construct(){
        ...
        $this->document_url = 'http://www.ooosibmetall.ru/sites/default/files/price_list/prays_na_ves_metalloprokat_dlya_sayta.xls';
        //$this->coef = 1000;
        $this->dual_cost = true;
        $this->price_id = 8859369;
    }
        ...
        foreach($this->sheet as $rows){
            ...
            foreach($rows as $num_row => $row){
                if(!empty($row)){
                    ...
                    if(count($rows) == 1){
                        if(in_array($row, array('Лента','Полоса','Лист стальной','Квадраты','Круги','Поковка','Шестигранник','Кольца','Колеса подкрановые','Вал','Лист','Труба',))){
                            $header = '';
                            continue;
                        }
                        $header = trim($row);
                    ...
                    if($num_row == $this->filter['horizontal']['to']['numeric']-1 && !in_array($row, array('мп'))){
                         continue;
                    }
                    if(in_array($row, array('тонна', 'труба'))){
                        continue;
                    }
                    $name .= ' '.$row;
                }
            ...
            if(!empty($name)&& !empty($cost)){
                $this->items[] = array('name' => preg_replace('/\?/iu','', preg_replace('/;/iu',',', preg_replace('/\s+/iu',' ', $name))), 'cost' => array($cost));
            }
            $name = '';
            $cost = '';
