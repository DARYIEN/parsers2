...
class CParFerrum extends CParMain{
    var $city_id;
    ...
    function __construct(){
        ...
        $this->document_urls['sort'] = 'http://ferrum-n.ru/images/price281113.xls';
        //$this->coef = 1000;
        $this->dual_cost = true;
        $this->price_id = 11270483;
        $this->author = 'Феликс';
        $this->iconv = true;
    }
        ...
        foreach($this->sheet as $rows){
            ...
            foreach($rows as $num_row => $row){
                if(!empty($row)){
                    ...
                    $new_header=false;
                    if(count($rows)>1 &&!isset($rows[$this->filter['horizontal']['to']['numeric'] - $this->filter['cost3']])){
                        continue 2;
                    }
            ...
            if(!empty($name)&& !empty($costs)){
                $this->items[] = array('name' => preg_replace('/Æ/iu','',preg_replace('/ø/iu','',preg_replace('/\?/iu','', preg_replace('/;/iu','!', preg_replace('/\s+/iu',' ', $this->filter['fake'].' '.$name.' '.$this->filter['end']))))), 'cost' => $costs);
            }
            $name = '';
            $cost = '';
            unset($costs);
