...
class CParNTPZ extends CParMain{
    var $city_id;
    var $message = '';
    var $document_urls;
    var $home_url;
    ...
    function __construct(){
        ...
        $this->dual_cost = true;
        $this->author = 'Михаил';
        $this->document_urls['one'] = 'http://ntpz.ru/catalog/price.php';
    ...
    function processParsing(){
        foreach($this->document_list as $key => $path){
            if($key == "one"){
                $this->filter = array('cost' => 1, '!hide' => array(0,1),'hide' => array(), 'title' => 0);
                $this->documentParsing($path);

                $this->filter = array('cost' => 3, '!hide' => array(1,2,3),'hide' => array(1), 'title' => 1);
                $this->documentParsing($path);

                $this->filter = array('cost' => 5, '!hide' => array(2,3,4,5),'hide' => array(2,3), 'title' => 2);
                $this->documentParsing($path);
            }
        ...
        if(!empty($table)){
            ...
            foreach ($el->find('tr') as $tr) {
                ...
                foreach ($el_td as $key => $value) {
                    ...
                    if(preg_match('/ТРУБА/ui', $td->text()) && $key == $this->filter['title']){
                        $title = $td->text();
                        //print_r($title);
                        $changeTitle = true;
                        continue;
                    }
                    if(in_array($key, $this->filter['hide']) && !$changeTitle) {
                        //print_r($td->text());
                        continue;
                    }
