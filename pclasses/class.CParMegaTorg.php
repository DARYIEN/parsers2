...
class CParMegaTorg extends CParMain{
    var $city_id;
    ...
    function __construct(){
        ...
        $this->document_urls['black'] = 'http://www.ksm095.ru/price-KSM095.xlsx';
        //$this->coef = 1000;
        $this->dual_cost = true;
        $this->price_id = 1995;
    }
    ...
    function processParsing(){
        foreach($this->document_list as $key => $path){
            if($key == 'black'){
                $pagesFirstType = [0,2,5,9,10,13,14];
                $pagesSecondType = [1,4];
                $pagesThirdType = [3,5,7,12,15,16];
                $pagesFourthType = [8,11];
                foreach($pagesFirstType as $numberOfList){
                    $this->filter =  array(
                        'propusk' => $this->num('BC'),
                        'cost1' => false,
                        'cost2' => 0,
                    ...
                    $this->sheet = $this->objPHPExcel->getSheet($numberOfList)->toArray();
                    $this->documentParsing();
                }
                foreach($pagesSecondType as $numberOfList){
                    $this->filter =  array(
                        'propusk' => $this->num('BC'),
                        'cost1' => false,
                        'cost2' => 1,
                ...
                foreach($pagesThirdType as $numberOfList){
                    $this->filter =  array(
                        'propusk' => $this->num('BC'),
                        'cost1' => false,
                        'cost2' => 1,
                ...
                foreach($pagesFourthType as $numberOfList){
                    $this->filter =  array(
        ...
        foreach($this->sheet as $rows){
            ...
            foreach($rows as $num_row => $row){
                if(!empty($row)){
                    ...
                    if(count($rows) == 1){
                        if(strripos($row,'ширина')!==false)continue;
                        if(strripos($row,'4ПБ')!==false)continue;
