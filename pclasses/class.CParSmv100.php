...
class CParSmv100 extends CParMain{
    var $city_id;
    ...
    function __construct(){
        ...
        $this->document_urls = $this->getUrl('http://smv100.ru/');
        ...
        foreach($this->sheet as $rows){
            ...
            foreach($rows as $num_row => $row){
                if(!empty($row)){
                    ...
                    if($this->filter['cost4']!==false && in_array($num_row, array($this->filter['horizontal']['to']['numeric'] - $this->filter['cost4']))){
                        $cost = str_replace(array(' ','руб.',',','р.','От','от','&#160;'),'',$row) * $this->filter['coef'];
                        if(!is_numeric($cost) || is_null($cost)){
                            $name = '';
                            $cost = '';
