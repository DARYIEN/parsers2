 ...
 * Time: 16:57
 */
class CParOKZ extends CParMain
{
    var $city_id;
    static $name_parser = array(
        'okz' => 'Объединенные керамические заводы'
    );
    public $items = array();
            ...
            foreach ($rows as $num_row => $row) {
                if (!empty($row)) {
                    ...
                    if (count($rows) == 1 && $this->filter['horizontal']['from']['numeric'] == $num_row) {
                        if (strripos($row, 'МАРКА') !== false) continue;
                        if (strripos($row, 'Марка') !== false) continue;
                        if ($new_header && $this->filter['skleyka']) {
                            $header .= ' ' . $row;
                            continue;
                        }
