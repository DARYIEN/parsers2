...
class CParRosmetal extends CParMain{
    var $city_id;
    ...
    function __construct(){
        ...
        $this->document_url = 'http://rosmetal.ru/files/price.xls';
        //$this->coef = 1000;
        $this->dual_cost = true;
        $this->price_id = 10646222;
    }
        ...
        foreach($this->sheet as $rows){
            ...
            foreach($rows as $num_row => $row){
                if(!empty($row)){
                    ...
                    if(!strripos($row,'L=')&&(count($rows) == 1 || in_array($row, array('АРМАТУРА ГЛАДКАЯ', 'ТРУБА  ПРОФ   ГОСТ 8645, 8639')))){
                        $header = $row;
                        continue;
                    }
