...
class CParTKBSD extends CParMain{
    var $city_id;
    static $name_parser = array(
        'tkbsd' => 'БашСтройДеталь'
    );
    public $items = array();
    ...
    function documentParsing(){
        ...
        foreach($this->sheet as $rows){
            ...
            if(!empty($name)&& !empty($costs)){
                $this->items[] = array('name' => preg_replace('/\(.*\)/','',preg_replace('/□/iu','',preg_replace('/Ǿ□/iu','',preg_replace('/\?/iu','', preg_replace('/;/iu','!', preg_replace('/\s+/iu',' ',trim($header.' '.$treeString.' '.$name.' '.$this->filter['end']))))))), 'cost' => $costs);
            }
            $name = '';
            $cost = '';
            unset($costs);
    ...
    public function getUrl(){
        $parse = file_get_html('http://www.uptkbsd.ru/torgovlya');
        $link = $parse->find('div.content p span span a', 0);
        return $link->href;
    }
}