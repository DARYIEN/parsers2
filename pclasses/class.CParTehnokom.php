...
class CParTehnokom extends CParMain{
    var $city_id;
    ...
    function __construct(){
        ...
        $this->document_url = 'http://www.trubotvod.ru/price/price-technokom.xls';
        //$this->coef = 1000;
        $this->dual_cost = false;
        $this->price_id = 10707803;
    }
        ...
        foreach($this->sheet as $rows){
            ...
            foreach($rows as $num_row => $row){
                if(!empty($row)){
                    ...
                    if($refresh==true && count($rows) == 1)$header = '';
                    if(count($rows) == 1){
                        $header .= ' '.trim(str_replace('Ду','',$row));
                        $refresh=false;
                        continue;
                    }
                    if($num_row == 0 && !is_numeric($rows[1])){
                        $header = str_replace($old,'',$header);
                        $header .= ' '.trim(str_replace(array('Размер','Ду'),'',$row));
                        $old = trim($row);
                        continue;
                    }
                    if($num_row == $this->filter['horizontal']['from']['numeric'] ) $name =str_replace('Цены на ','',$header);
