...
class CParDarStroy extends CParMain{
    var $city_id;
    var $message = '';
    var $document_urls;
    var $home_url;
    ...
    function __construct(){
        ...
        $this->dual_cost = true;
        $this->author = 'Феликс';
        $this->document_urls['all'] = 'http://www.sto-pudov.ru/price.htm';
    ...
    function processParsing(){
        foreach($this->document_list as $key => $path){
            if($key == "all"){
                $this->filter = array('cost' => [4], 'hide' => [0,3], 'coef' => 1, 'selector' => 'table tr td div', 'count' => 5);
                $this->documentParsing($path);
                $this->filter = array('cost' => [3], 'hide' => [0], 'coef' => 1, 'selector' => 'table tr td div', 'count' => 4);
                $this->documentParsing($path);
                $this->filter = array('cost' => [7], 'hide' => [0,6], 'coef' => 1, 'selector' => 'table tr td div', 'count' => 7);
                $this->documentParsing($path);
            }
   ...
   /* function getDocuments(){
    ...
    function documentParsing($path){
        $parse = phpQuery::newDocumentFileHTML($path,"ISO-8859-1");
        //echo $parse->charset;
