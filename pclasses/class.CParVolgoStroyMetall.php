...
class CParVolgoStroyMetall extends CParMain{
    var $city_id;
    static $name_parser = array(
        'volgostroymetall' => 'ВолгоСтройМеталл'
    );
    public $items = array();
    function start(){
    ...
    function __construct(){
        ...
        //$this->document_urls['main'] = $this->getUrl();
        $this->document_urls['main'] = 'http://vsm34.ru/stroyka.xls';
        $this->document_urls['metall'] = 'http://vsm34.ru/price_metal.xls';
        //$this->coef = 1000;
        $this->dual_cost = true;
        $this->price_id = 11346354;
        $this->author = 'Михаил';
    }
        ...
        foreach($this->document_list as $key => $path){
            if($key == 'main'){
                ...
                $this->filter =  array(
                    ...
                    'cost2' => 0,
                    'coef' => 1,
                    'end' => 'ТРУБЫ БЕСШОВНЫЕ ГОРЯЧЕДЕФОР-МИРОВАННЫЕ',
                    'header' => 'ГОСТ 8732-78',
                    'skleyka' => false,
                    'dualheader'=>false,
                    'tree' => false,
        ...
        foreach($this->sheet as $rows){
            ...
            foreach($rows as $num_row => $row){
                if(!empty($row)){
                    ...
                    if($this->filter['fixcell']!== false && !empty($rows[$this->filter['fixcell']])){
                        $fixCell=$row;
                        continue;
                    }
                    if($this->filter['fixcell']!== false && in_array($num_row-$this->filter['baseOfFixCell'], $this->filter['fixcell'])){
                        $name .= ' '.$fixCell;
                    }
            ...
            if(!empty($name)&& !empty($costs)){
                $this->items[] = array('name' => preg_replace('/Ǿ/iu','',preg_replace('/˚/iu','',preg_replace('/\?/iu','', preg_replace('/;/iu','!', preg_replace('/\s+/iu',' ',trim($header.' '.$treeString.' '.$name.' '.$this->filter['end'])))))), 'cost' => $costs);
            }
            $name = '';
            $cost = '';
            unset($costs);
