...
class CParEVRASPerm extends CParMain{
    var $city_id;
    ...
    function __construct(){
        ...
        $this->document_url = 'http://www.emural.ru/files/0/84686121372843828.xls';
        //$this->coef = 1000;
        $this->dual_cost = true;
        $this->price_id = 10662977;
    }
    ...
    function documentParsing(){
        ...
        foreach($this->sheet as $rows){
            ...
            foreach($rows as $num_row => $row){
                if(!empty($row)){
                    if(in_array($num_row, array($this->filter['horizontal']['to']['numeric']))){
                        ...
                        //if($cost==0) continue;
                        $costs[] = round($cost * round(100/(100-10),4),-1);
                        continue;
                    }
                    if(in_array($num_row, array($this->filter['horizontal']['to']['numeric']-/*$this->filter['dermolist']*/3))){
                        $cost = (int) str_replace(' ','',$row);
                        if(!is_numeric($cost) || is_null($cost)){
                            $name = '';
                            $cost = '';
                    ...
                    if($num_row == $this->filter['horizontal']['to']['numeric']-1 || $num_row == $this->filter['horizontal']['to']['numeric']-2){
                        continue;
                    }
                    ...
                    if(!empty($rows[$num_row])&&$num_row == $this->filter['horizontal']['from']['numeric']){
                        $header2 = trim($row);
                      //  $name = $header2;
                       // continue;
                    }
            ...
            if(!empty($name)&& !empty($costs)&&!empty($header2)){
