...
class CParStalintexTreid extends CParMain{
    var $city_id;
    static $name_parser = array(
        'stalintextreid' => 'Стальинтекс Трейд'
    );
    public $items = array();
    function start(){
        ...
        foreach($this->sheet as $rows){
            ...
            foreach($rows as $num_row => $row){
                if(!empty($row)){
                    ...
                    if(in_array($row, array('Руб\м','Руб\тн'))){
                        $header = $rows[$this->filter['horizontal']['from']['numeric']];
                    }
            ...
            if(!empty($name)&& !empty($costs)){
                $this->items[] = array('name' => preg_replace('/\s+/iu',' ',preg_replace('/Ǿ/iu','',preg_replace('/\?/iu','', preg_replace('/;/iu','!', preg_replace('/"/iu',' ',trim($this->filter['begin'].' '.$header.' '.$treeString.' '.$name.' '.$this->filter['end'])))))), 'cost' => $costs);
            }
            $name = '';
            $cost = '';
            unset($costs);
