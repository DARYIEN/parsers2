...
class CParUglerod extends CParMain{
    var $city_id;
    ...
    function __construct(){
        ...
        $this->document_urls['list'] = 'http://uglerod-rostov.ru/assets/files/List%20-%20Uglerod.xls';
        $this->document_urls['krug'] = 'http://uglerod-rostov.ru/assets/files/Price%20Krug.xlsx';
        //$this->document_urls['krepezh'] = 'http://www.zitar.ru/files/price/64/%CA%F0%E5%EF%E5%E6%ED%E0%FF%20%F2%E5%F5%ED%E8%EA%E0.XLS';
        //$this->coef = 1000;
        $this->dual_cost = true;
        $this->price_id = 11350308;
        $this->author = 'Михаил';
    }
            ...
            foreach($rows as $num_row => $row){
                if(!empty($row)){
                    ...
                    if($this->filter['cost1']!==false && in_array($num_row, array($this->filter['horizontal']['to']['numeric'] - $this->filter['cost1']))){
                        $cost = Round(str_replace(array(' ','руб.',',','р.','от'),'',$row) * $this->filter['coef']);
