...
class CParStalnoyDom extends CParMain{
    var $city_id;
    static $name_parser = array(
        'stalnoyDom' => 'Стальной Дом'
    );
    public $items = array();
    function start(){
        ...
        foreach($this->sheet as $rows){
            ...
            foreach($rows as $num_row => $row){
                if(!empty($row)){
                    ...
                    if(in_array(end($rows), array('кг'))){
                        $this->filter['coef'] = 1000;
                    }
                    if(in_array(end($rows), array('т','тн','м. п.','м','шт'))){
                        $this->filter['coef'] = 1;
                    }
    ...
    public function getUrl(){
        $hrefs = array();
        foreach(file_get_html('http://stalnoydom.ru/')->find('li.level-1 a.level-1') as $link){
            $href = 'http://stalnoydom.ru'.$link->href;
            foreach(file_get_html($href)->find('a') as $link2){
                if(strripos($link2->href, '/local/ajax/makeExel/download.php')!== false){
                    $href = 'http://stalnoydom.ru'.$link2->href;
                    $hrefs[] = $href;
                }
            }
        }
