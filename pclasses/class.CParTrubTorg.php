...
class CParTrubTorg extends CParMain{
    var $city_id;
    static $name_parser = array(
        'trubtorgmoskow' => 'ТРУБОТОРГ'
    );
    public $items = array();
    function start(){
    ...
    public function getUrl(){
        /*preg_match_all('~href=\W\S+.xlsx\W~', file_get_html('http://www.trubotorgrus.ru/prays-list-nerzhaveyushhiy')->find('body',0)->innertext, $out);
        $links['list']=$out[0][0];
        preg_match_all('~href=\W\S+setka.xlsx\W~', file_get_html('http://www.trubotorgrus.ru/prays-list')->find('body',0)->innertext, $out);
        $links['setka']='http://www.trubotorgrus.ru'.$out[0][0];
        preg_match_all('~href=\W\S+.xlsx\W~', file_get_html('http://www.trubotorgrus.ru/prays-truba')->find('body',0)->innertext, $out);
        $links['truba']=$out[0][0];
        $out = file_get_html('http://www.trubotorgrus.ru/prays-krug-i-shestigrannik')->find('body div.art-postcontent h2 a',0)->href;
        $links['krug']=$out;
        preg_match_all('~href=\W\S+.xlsx\W~', file_get_html('http://www.trubotorgrus.ru/prays-otvodyi')->find('body',0)->innertext, $out);
        $links['otvod']=$out[0][0];
        preg_match_all('~href=\W\S+.xlsx\W~', file_get_html('http://www.trubotorgrus.ru/prays-provoloka')->find('body',0)->innertext, $out);
        $links['provoloka']=$out[0][0];
        preg_match_all('~href=\W\S+.xls\W~', file_get_html('http://www.trubotorgrus.ru/kranyi-prays')->find('body',0)->innertext, $out);
        $links['kran']=$out[0][0];
        $links=str_replace(array('href=','"',"'"), '', $links);*/
        echo 'ошибка, надо урлы поправить';
        //p($links);
        //die();
        $links = '';
        return $links;
...
class CParTrubTorgUfa extends CParTrubTorg{
    function __construct(){
        parent::__construct();
        $this->price_id = 11487214;
        $this->document_name= 'TrubTorgUfa'.'_'.date('d-m-Y', time()).'_'.time().'.csv';
    }
}
class CParTrubTorgKrasnodar extends CParTrubTorg{
    function __construct(){
        parent::__construct();
        $this->price_id = 11487217;
        $this->document_name= 'TrubTorgKrasnodar'.'_'.date('d-m-Y', time()).'_'.time().'.csv';
    }
}