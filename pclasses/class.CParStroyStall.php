...
class CParStroyStall extends CParMain{
    var $city_id;
    static $name_parser = array(
        'stroystall' => 'СТРОЙСТАЛЬ'
    );
    function start(){
    ...
    function __construct(){
        ...
        $this->document_url = 'http://stroymetall.ru/docs/price.xls';
        //$this->coef = 1000;
        $this->dual_cost = true;
        $this->price_id = 7278594;
    }
        ...
        foreach($this->sheet as $rows){
            ...
            foreach($rows as $num_row => $row){
                if(!empty($row)){
                    ...
                    if($num_row == $this->filter['horizontal']['from']['numeric'] + 1 ) $name =$header;
                    $name .= ' '.$row;
                }
            }
