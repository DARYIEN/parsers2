...
class CParRosGrup extends CParMain{
    var $city_id;
    var $message = '';
    var $document_urls;
    var $home_url;
    ...
    function __construct(){
        ...
        $this->document_urls = array(   'http://rosgrup.ru/price',
                                    );
        $this->home_url = current($this->document_urls);
        $this->price_id = 8532544;
        $this->iconv = true;
    }
    ...
    function documentParsing($path){
        ...
        $headers = $parse->find('caption');
        $tables = $parse->find('table');
        if(!empty($headers)){
            foreach($headers as $header){
                foreach($header->find('span') as $content){
                    if (trim($content->plaintext) == 'ГОСТ 3262-75') continue;
                    $text[]=trim($content->innertext);
                ...
                foreach($table->find('tr') as $tr){
                    ...
                    foreach($tr->find('td') as $td){
                        ...
                        if($step == 1){
                            ...
                            if ($td->innertext == '09Г2С'){
                                $name=$fixName.' '.$td->innertext;
                                $step = $end_cell['cost_cell1'];
                                continue;
                            }
                            if (trim($td->plaintext) == '10 х 10' && $key ==5) $text[$key].= ' ГОСТ 30245-03 квадратные';
                            if (trim($td->plaintext) == '30 х 20' && $key ==5) $text[$key]= ' Трубы электросварные профильные ГОСТ 30245-03 прямоугольные';
                            $name = $text[$key].' '.$td->innertext;
                            if (trim($td->plaintext) == 'Ду 80' && $key == 4) $text[$key]='Трубы стальные водогазопроводные ГОСТ 3262-75 оц.';
                        }else if ($step >= 4 && $step <= $end_cell['stop_cell']){
                            $name .= ' '.$td->innertext;
                            if ($step == 5) $fixName=$name;
