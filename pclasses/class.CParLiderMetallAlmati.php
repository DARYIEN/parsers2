...
class CParLiderMetallAlmati extends CParMain{
    var $city_id;
    static $name_parser = array(
        'lidermetallalmati' => 'Лидер-Металл'
    );
    public $items = array();
    function start(){
        ...
        foreach($this->sheet as $rows){
            ...
            foreach($rows as $num_row => $row){
                if(!empty($row)){
                    ...
                    if($num_row == $this->filter['horizontal']['from']['numeric'] && count($rows) <= 2){
                        if($row == '50х50х2') continue;
