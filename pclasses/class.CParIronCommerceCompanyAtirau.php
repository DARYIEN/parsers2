...
class CParIronCommerceCompanyAtirau extends CParMain{
    var $city_id;
    static $name_parser = array(
        'ironcommercecompanyatyrau' => 'IRON COMMERCE Атырау'
    );
    public $items = array();
    function start(){
        ...
        foreach($this->sheet as $rows){
            ...
            foreach($rows as $num_row => $row){
                if(!empty($row)){
                    ...
                    if($num_row == $this->filter['horizontal']['from']['numeric'] && !is_numeric($rows[2])){
