...
class CParCosmoplast extends CParMain{
    var $city_id;
    ...
    function __construct(){
        ...
        $this->document_urls['main'] = 'http://www.cosmoplast.ru/data/File/vnutr-kanal-price-05112013.xls';
        $this->document_urls['pvh'] = 'http://www.cosmoplast.ru/data/File/Cosmplast_PVC_OF_THE_PIPE_AND_THE_FITTING_FOR_THE_OUTSIDE_SEWERAGE(3).xls';
        $this->document_urls['ppr'] = 'http://www.cosmoplast.ru/data/File/Cosmplast_POLIPROPILENOVYE_(PPR)_PIPES_AND_THE_FITTING_FOR_HEATING_AND_WATER-SUPPLY(2).xls';
        //$this->document_urls['krepezh'] = 'http://www.zitar.ru/files/price/64/%CA%F0%E5%EF%E5%E6%ED%E0%FF%20%F2%E5%F5%ED%E8%EA%E0.XLS';
        //$this->coef = 1000;
        $this->dual_cost = true;
        $this->price_id = 99929993999;
        $this->author = 'Михаил';
    }
    ...
    function documentParsing(){
        ...
        foreach($this->sheet as $rows){
            ...
            if(!empty($name)&& !empty($costs)){
                $this->items[] = array('name' => preg_replace('/Ǿ/iu','',preg_replace('/\?/iu','', preg_replace('/;/iu','!', preg_replace('/\s+/iu',' ',trim($header.' '.$fixCell.' '.$treeString.' '.$name.' '.$this->filter['end']))))), 'cost' => $costs);
            }
            $name = '';
            $cost = '';
            unset($costs);
