...
class CParPromEx extends CParMain{
    var $city_id;
    ...
    function documentParsing(){
        ...
        foreach($this->sheet as $rows){
            ...
            if(!empty($name)&& !empty($costs)){
                $this->items[] = array('name' => preg_replace('/½/iu','1/2',preg_replace('/¼/iu','1/4',preg_replace('/□/iu','',preg_replace('/Ǿ□/iu','',preg_replace('/\?/iu','', preg_replace('/;/iu','!', preg_replace('/\s+/iu',' ',trim($this->filter['fake'].$header.' '.$treeString.' '.$name.' '.$this->filter['end'])))))))), 'cost' => $costs);
            }
            $name = '';
            $cost = '';
            unset($costs);
    ...
    public function getUrl(){
        $parse = file_get_html('http://www.prom-ex.ru/');
        $link = $parse->find('div#content div#price a', 0);
        return 'http://www.prom-ex.ru'.$link->href;
    }
}