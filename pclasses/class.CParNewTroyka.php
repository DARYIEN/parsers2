 ...
 * Time: 12:44
 */
class CParNewTroyka extends CParMain{
    var $city_id;
    var $message = '';
    var $document_urls;
    var $home_url;
    ...
    function __construct(){
        ...
        $this->home_url = 'http://www.troykastal.ru/catalog-setka-svarnaya';
    ...
    function documentParsing($path){
        ...
        $table = $parse->find('table tbody tr');
        if(!empty($table)){
            $result = array();
            foreach($table as $tr){
                $temprorary = array();
                $temp = array();
                $i = 1;
                foreach($tr->find('td.cent') as $tdcost){
                    //p($span->innertext);
                    //echo iconv('cp1251','utf-8',$td->innertext);
                    if($i == 2){
                        $temp['cost'] =  trim($tdcost->innertext);
                    }
                    $i++;
                }
                $td = $tr->find('td',0);
                $temp['name'] =  trim($td->innertext);
    ...
    public function getUrl(){
        ...
        $info_all = $parse->find('ul.menu li');
        $href = array();
        foreach($info_all as $info_first){
            $info_second = $info_first->find('a',0);
            $href[] = 'http://www.troykastal.ru'.$info_second->href;
            //break;
        }
