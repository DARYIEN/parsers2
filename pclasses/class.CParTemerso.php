...
class CParTemerso extends CParMain{
    var $city_id;
        ...
        foreach($this->sheet as $rows){
            ...
            foreach($rows as $num_row => $row){
                if(!empty($row)){
                    ...
                    if((count($rows) == 1 || (count($rows) == 3 && isset($rows[11]) && $rows[11]==39000)) && $this->filter['horizontal']['from']['numeric'] == $num_row && $this->filter['header'] === true){
                    ...
                    if(count($rows) == 3 && isset($rows[11]) && $rows[11]==39000){
                        continue 2;
                    }
                    $new_header=false;
                    $name .= ' '.$row;
    ...
    public function getUrl(){
        $parse = file_get_html('http://www.zaotemerso.ru/');
        $link = $parse->find('div.fb_index p a', 0);
        return 'http://www.zaotemerso.ru'.$link->href;
    }
}