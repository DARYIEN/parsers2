...
class CParTehGrup extends CParMain{
    var $city_id;
        ...
        foreach($this->document_list as $key => $path){
            if($key == 'main'){
                ...
                $this->filter =  array(
                    ...
                    'coef' => 1,
                    'end' => '',
                    'header' => 'Прокат конструкционный никельсодржащий',
                    'skleyka' => false,
                    'dualheader'=>false,
                    'tree' => false,
                    'fixcell' => false,
    ...
    public function getUrl(){
        $parse = file_get_html('http://www.metaloptom.ru/');
        $links = 'http://www.metaloptom.ru'.$parse->find('td.menuNormal a', 3)->href;
        //p($links);
        return $links;
    }
}