...
class CParUkspar extends CParMain{
    var $city_id;
    ...
    function __construct(){
        ...
        $this->document_urls['black'] = 'http://ukspar.ua/downloads/price/price_ukspar.xls';
        //$this->coef = 1000;
        $this->dual_cost = false;
        $this->decimal = true;
        $this->price_id = 11487224;
    }
            ...
            foreach($rows as $num_row => $row){
                if(!empty($row)){
                    ...
                    if(in_array($num_row, array($this->filter['horizontal']['to']['numeric'] - $this->filter['cost2']))){
                        $cost = str_replace(array(' ','руб.',',','р.','грн'),'',$row) * $this->filter['coef'];
                        if(!is_numeric($cost) || is_null($cost)){
                            $name = '';
                            $cost = '';
                            continue 2;
