...
class CParEVRAS_type2 extends CParMain{
    var $city_id;
    ...
    function documentParsing(){
        ...
        foreach($this->sheet as $rows){
            ...
            if(empty($header) && !empty($rows)){
                $header=$rows[$this->filter['horizontal']['from']['numeric']].' '.$rows[$this->filter['horizontal']['from']['numeric'] + $this->filter['dermolist']];
                continue;
            }
            $name = $header;
            foreach($rows as $num_row => $row){
                if(!empty($row)){
                    ...
                    if($this->filter['dermolist'] > 1 && $this->filter['horizontal']['from']['numeric']+1 == $num_row){
                        continue;
                    }
...
class CParEVRASUfa extends CParEVRAS_type2{
    function __construct(){
        parent::__construct();
        $this->document_url = 'http://www.emural.ru/files/0/128361571402901838.xls';
        $this->price_id = 10662959;
    }
}