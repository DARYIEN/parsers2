...
class CParMetTehnoN extends CParMain{
    var $city_id;
    static $name_parser = array(
        'mettehnon' => 'МетТехно-Н'
    );
    public $items = array();
    function start(){
    ...
    function __construct(){
        ...
        $this->document_urls['main'] = 'http://mettehno-n.ru/price/%D0%BF%D1%80%D0%B0%D0%B9%D1%81_%D0%BE%D0%B1%D1%89%D0%B8%D0%B9.xls';
        //$this->coef = 1000;
        $this->dual_cost = true;
        $this->price_id = 11367530;
        $this->author = 'Михаил';
    }
    ...
    function documentParsing(){
        ...
        foreach($this->sheet as $rows){
            ...
            if(!empty($name)&& !empty($costs)){
                $this->items[] = array('name' => preg_replace('/фо\D{0,2}/iu','',preg_replace('/\?/iu','', preg_replace('/;/iu','!', preg_replace('/\s+/iu',' ',trim($header.' '.$treeString.' '.$name.' '.$this->filter['end']))))), 'cost' => $costs);
            }
            $name = '';
            $cost = '';
            unset($costs);
