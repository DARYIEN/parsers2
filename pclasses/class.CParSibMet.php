...
class CParSibMet extends CParMain{
    var $city_id;
    static $name_parser = array(
        'sibmet' => 'Сибирский металл'
    );
    function start(){
    ...
    function __construct(){
        ...
        $this->document_url = 'http://sibmet.com/price/price.xls';
        //$this->coef = 1000;
        $this->dual_cost = false;
        $this->price_id = 10683471;
    }
            ...
            foreach($rows as $num_row => $row){
                if(!empty($row)){
                    if(in_array($num_row, array($this->filter['horizontal']['to']['numeric']))){
                        $cost = Round(( str_replace(' ','',preg_replace('~[^0-9]+~','',$row)))/100);
