...
class CParPromRukav extends CParMain{
    var $city_id;
    var $message = '';
    var $document_urls;
    var $home_url;
    ...
    function __construct(){
        ...
        $this->dual_cost = true;
        $this->author = 'Михаил';
        $this->document_urls = $this->getUrl('http://www.promrukav.ru/price/');
    ...
    function documentParsing($path){
        ...
        $tables = $parse->find('table table table');
//        if(!empty($headers)){
...
//        }
            ...
            foreach($tables as $key=>$table){
                ...
                foreach($table->find('tr') as $tr){
                    ...
                    foreach($tr->find('td') as $key =>  $td){
                        if(count($tr->find('td')) > 1 && $key == count($tr->find('td')) - 2) {
                            $td->find('span.razdel',0)->outertext = '';
                            //echo(str_get_html($td)->plaintext);
                            $cost = str_replace(array(' ', 'руб.', 'р.', '&#160;', '&nbsp;', chr(160), 'От', 'от'), '', str_get_html($td)->plaintext);
    ...
    public function getUrl($url){
        ...
        $info_all = $parse->find('select[name=product] option');
        $hrefs = array();
        foreach($info_all as $info_first){
            //$info_second = $info_first->value;
            $link = $info_first->value;
            if($link != '/'){
