...
class CParSteelTorg extends CParMain{
    var $city_id;
    var $message = '';
    var $document_urls;
    var $home_url;
    ...
    function __construct(){
        ...
        $this->dual_cost = true;
        $this->author = 'Михаил';
        $this->document_urls = $this->getUrl('http://www.steel-torg.com/metalloprokat/armatura');
    ...
    function documentParsing($path){
        ...
        $tables = $parse->find('article.grid_11 table');
//        if(!empty($headers)){
...
//        }
            ...
            foreach($tables as $key=>$table){
                ...
                foreach($table->find('tr') as $tr){
                    ...
                    foreach($tr->find('td') as $key =>  $td){
                        ...
                        if($key == count($tr->find('td')) - 2) continue;
                        $name .= ' '.$td->plaintext;
                    }
    ...
    public function getUrl($url){
        ...
        $info_all = $parse->find('ul li ul li ul li ul li');
        $hrefs = array();
        foreach($info_all as $info_first){
            $info_second = $info_first->find('a',0);
            $link = $info_second->href;
            if($link != '/'){
                if(strripos($link, parse_url($url, PHP_URL_HOST))!== true){
