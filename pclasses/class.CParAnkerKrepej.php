...
class CParAnkerKrepej extends CParMain{
    var $city_id;
    static $name_parser = array(
        'ankerkrepej' => 'Анкер-Крепеж'
    );
    public $items = array();
    function start(){
        ...
        foreach($this->sheet as $rows){
            ...
            foreach($rows as $num_row => $row){
                if(!empty($row)){
                    ...
                    if(in_array(trim($row), array('руб\1шт.'))){
                        $this->filter['end'] = $row;
                    }
                    if(in_array(trim($row), array('РУБЛЕЙ за 1 000 штук'))){
                        $this->filter['end'] = $row;
                    }
                    if(in_array(trim($row), array('РУБЛЕЙ за штуку'))){
                        $this->filter['end'] = $row;
                    }
    ...
    public function getUrl(){
        preg_match_all('~href=\W\S+.xlsx{0,1}\W~', file_get_html('http://ankerfix.ru/price.php')->find('body',0)->innertext, $out);
        $links['main']='http://ankerfix.ru'.$out[0][0];
