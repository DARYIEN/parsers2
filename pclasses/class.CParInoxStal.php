...
class CParInoxStal extends CParMain{
    var $city_id;
    ...
    function __construct(){
        ...
        $this->document_urls['CParInoxStal'] = 'http://inoxstal.ru/upload/price_inoxstal.xlsx';
        //$this->coef = 1000;
        $this->dual_cost = true;
        $this->price_id = 10618328;
    }
        ...
        foreach($this->sheet as $rows){
            ...
            foreach($rows as $num_row => $row){
                if(!empty($row)){
                    ...
                    if(in_array($row, array('AISI 316Ti','Полоса',))){
                        $this->filter['header'] = true;
                    }
                    if(in_array($row, array('AISI 304',))){
                        $this->filter['preheader'] = '';
                    }
                    if(count($rows) == 1 ){
            ...
            if(!empty($name)&& !empty($costs)){
                $this->items[] = array('name' => preg_replace('/ø/iu','',preg_replace('/\?/iu','', preg_replace('/;/iu','!', preg_replace('/\s+/iu',' ', $this->filter['preheader'].' '.$name.' '.$this->filter['end'])))), 'cost' => $costs);
            }
            $name = '';
            $cost = '';
            unset($costs);
