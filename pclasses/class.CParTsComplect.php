...
class CParTsComplect extends CParMain{
    var $city_id;
    static $name_parser = array(
        'tscomplect' => 'Тex Строй комплект.'
    );
    public $items = array();
    function start(){
    ...
    function __construct(){
        ...
        $this->document_urls['main'] = 'http://www.tscomplect.ru/files///Prise%20TSK.xls';
        ...
        foreach($this->sheet as $rows){
            ...
            foreach($rows as $num_row => $row){
                if(!empty($row)){
                    ...
                    if(isset($rows[$this->filter['horizontal']['to']['numeric']]) && $rows[$this->filter['horizontal']['to']['numeric']] == 'кг'){
