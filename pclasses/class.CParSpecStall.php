...
class CParSpecStall extends CParMain{
    var $city_id;
    ...
    function __construct(){
        ...
        $this->document_url = $this->getUrl('http://td-zsss.ru/prays-list');
        //$this->coef = 1000;
        $this->dual_cost = true;
        $this->price_id = 7669926;
    }
        ...
        foreach($this->sheet as $rows){
            ...
            foreach($rows as $num_row => $row){
                if(!empty($row)){
                    ...
                    if(in_array($row, array('т', 'шт','28,0х1,5 мм DN20 3/4"','60,3,х48,3х2 мм DN50x40 2"x1 1/2"'))){
                        continue;
                    }
                    if(count($rows) == 1 ){
                        $header = trim($row);
            ...
            if($header=='Отводы нержавеющие') $coef=1;
            if($header=='Круг нержавеющий') $coef=1000;
            //Спецсталь нада *1000, кроме отводов, тройников , переходов, фланцев
