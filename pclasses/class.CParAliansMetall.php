...
class CParAliansMetall extends CParMain{
    var $city_id;
    static $name_parser = array(
        'aliansmetall' => 'Альянс-Металл'
    );
    public $items = array();
    function start(){
        ...
        foreach($this->sheet as $rows){
            ...
            foreach($rows as $num_row => $row){
                if(!empty($row)){
                    ...
                    if($this->filter['header'] && $this->filter['horizontal']['from']['numeric'] == $num_row  && count($rows) == 1 ){
    ...
    public function getUrl(){
        preg_match_all('~href=\W\S+.xlsx{0,1}\W~', file_get_html('http://alians-met.ru/price')->find('body',0)->innertext, $out);
        $links['main']='http://alians-met.ru'.$out[0][0];
