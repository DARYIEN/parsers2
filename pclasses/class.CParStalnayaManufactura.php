...
class CParStalnayaManufactura extends CParMain{
    var $city_id;
    static $name_parser = array(
        'stalnayamanufactura' => 'Стальная мануфактура'
    );
    public $items = array();
    function start(){
    ...
    function __construct(){
        ...
        $this->document_urls['black'] = 'http://metallocentr74.ru/upload/iblock/f6c/f6c50c16edc06359b807e0c6e38b1903.xls';
        //$this->coef = 1000;
        $this->dual_cost = true;
        $this->price_id = 8532280;
    }
            ...
            foreach($rows as $num_row => $row){
                if(!empty($row)){
                    if($this->filter['cost1']!==false && in_array($num_row, array($this->filter['horizontal']['to']['numeric'] - $this->filter['cost1']))){
                        $cost = str_replace(array(' ',chr(160)),'',preg_replace('/[^0-9]+\//','',iconv('UTF-8','windows-1251',$row))); //* $this->filter['coef'];
                        if(is_null($cost)){
                            $name = '';
                            $cost = '';
                            continue 2;
