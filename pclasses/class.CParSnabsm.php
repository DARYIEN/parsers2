...
class CParSnabsm extends CParMain{
    var $city_id;
    var $message = '';
    var $document_urls;
    var $home_url;
    ...
    function __construct(){
        ...
        $this->dual_cost = true;
        $this->author = 'Михаил';
        $this->document_urls = array('http://snabsm.ru/magazin/folder/plity-perekrytiya-kamer');
    ...
    function documentParsing($path){
        ...
        $tables = $parse->find('form.product-item-thumb');
        if(!empty($tables)){
            $result = array();
            $table_number  = 1;
            $header = '';
            foreach($tables as $key=>$table){
                $costs = array();
                $step = 1;
                $name = $table->find('.product-name a', 0)->plaintext;
                $cost = str_replace(array(' ', 'руб.', 'р.', '&#160;', '&nbsp;', chr(160), 'От', 'RUB'), '', $table->find('.price-current strong', 0)->plaintext);
