...
class CParChelyabMetallUralOptTorg extends CParMain{
    var $city_id;
    static $name_parser = array(
        'chelyabmetalluralopttorg' => 'ЧелябМЕталлОптТорг'
    );
    public $items = array();
    function start(){
    ...
    function __construct(){
        ...
        //$this->document_urls['izdelia'] = 'http://chelyabmetall.ru/netcat_files/239/205/Metalloizdeliya.xls';
        $this->document_urls['nekond'] = $this->getUrl();
        //$this->coef = 1000;
        $this->dual_cost = true;
        $this->price_id = 10922318;
    }
        ...
        foreach($this->sheet as $rows){
            ...
            foreach($rows as $num_row => $row){
                if(!empty($row)){
                    ...
                    if(in_array($row, array('ЗАКЛАДНЫЕ', 'СПЕЦПРЕДЛОЖЕНИЕ ПО ЗАКЛАДНЫМ - 28 000 руб/тн',))){
                        continue 2;
                    }
                    /*
