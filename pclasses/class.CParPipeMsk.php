...
class CParPipeMsk extends CParMain{
    var $city_id;
    var $message = '';
    var $document_urls;
    var $home_url;
    ...
    function __construct(){
        ...
        $this->dual_cost = true;
        $this->author = 'Михаил';
        $this->document_urls = $this->getUrl('http://pipe-msk.ru/catalog/');
        //$this->document_urls = array('http://pipe-msk.ru/catalog/product/8425-truby-vodogazoprovodnye/');
    ...
    function documentParsing($path){
        ...
        $tables = $parse->find('div.price-table table');
//        if(!empty($headers)){
...
//                    }
    ...
    public function getUrl($url){
        ...
        $info_all = $parse->find('div.leftmenu li a');
        $hrefs = array();
        ...
        foreach($hrefs as $href){
            ...
            $info_all = $parse->find('#catalog-list div.box_text h2 a');
            foreach($info_all as $info_first){
                //$info_second = $info_first->value;
                $link = $info_first->href;
                if($link != '/'){
