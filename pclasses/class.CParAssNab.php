...
class CParAssNab extends CParMain{
    var $city_id;
    ...
    function __construct(){
        ...
        //$this->document_urls = $this->getUrl('http://www.assnab.ru/pricelist/price.php');
        $this->document_urls['main'] = 'http://www.assnab.ru/pricelist/price.php';
            ...
            foreach($rows as $num_row => $row){
                if(!empty($row)){
                    ...
                    if(in_array($num_row, array($this->filter['horizontal']['to']['numeric'] - $this->filter['cost2']))){
                        ...
                        $cost = str_replace(array(' ','руб.','р.','&#160;','&nbsp;', chr(160),'От','от'), '', current(explode('руб.', $row))) * $this->filter['coef'];
                        if(!is_numeric($cost) || is_null($cost)){
                            $name = '';
                            $cost = '';
                            continue 2;
