...
class CParSanTehGrad extends CParMain{
    var $city_id;
    static $name_parser = array(
        'SanTehGrad' => 'SanTehGrad'
    );
    public $items = array();
    function start(){
    ...
    function __construct(){
        ...
        $this->document_urls['main'] = $this->getUrl('http://santehgrad.ru/prajs-list.html');
        ...
        foreach($this->sheet as $rows){
            ...
            foreach($rows as $num_row => $row){
                if(!empty($row)){
                    ...
                    if($this->filter['header']  && count($rows) == 2 && $this->filter['horizontal']['from']['numeric'] + 1  == $num_row){
