...
class CParMetalurgicheskayaKopaniya extends CParMain{
    var $city_id;
    static $name_parser = array(
        'metalurgicheskayakopaniya' => 'Металлургическая Компания'
    );
    public $items = array();
    function start(){
        ...
        foreach($this->sheet as $rows){
            ...
            foreach($rows as $num_row => $row){
                if(!empty($row)){
                    ...
                    if(in_array(trim($row), array('в наличии  уголки'))){
                        $header = 'в наличии  уголки';
                        continue;
                    }
                    if(in_array(trim($row), array('Труба электросварная тонкост'))){
                        $header = 'Труба электросварная тонкост';
                        continue;
                    }
                    if(in_array(trim($row), array('Уголок'))){
                        $header = $row;
                        continue;
                    }
                    if(in_array(trim($row), array('30 Б1, Б2, Ш1 дл.=12 м'))){
                        continue;
                    }


                    if($this->filter['header'] && $this->filter['horizontal']['from']['numeric'] == $num_row  && ((count($rows) == 1 )||strripos($rows[$this->filter['horizontal']['to']['numeric']],'от 3тн')!==false||strripos($rows[$this->filter['horizontal']['to']['numeric']],'от 1тн'!==false))){
