...
class CParTeploVodSnab extends CParMain{
    var $city_id;
    static $name_parser = array(
        'teplovodsnab' => 'Тепловодоснаб'
    );
    public $items = array();
    function start(){
    ...
    function __construct(){
        ...
        $this->document_urls['black'] = 'http://www.hitarm.ru/upload/iblock/ba7/ba7c3222a1a2638ab916ff767f84baf9.xls';
        //$this->coef = 1000;
        $this->dual_cost = true;
        $this->price_id = 8183174;
    }
            ...
            foreach($rows as $num_row => $row){
                if(!empty($row)){
                    if($this->filter['cost1']!==false && in_array($num_row, array($this->filter['horizontal']['to']['numeric'] - $this->filter['cost1']))){
                        if(strripos($row,'EUR') !== false){ $cost = Round(str_replace(array(' ','EUR'),'',$row)) * $this->filter['coef']; $name.= ' EUR';} else $cost = Round(str_replace(array(' ','руб.'),'',$row)) * $this->filter['coef'];
...
class CParTeploVodSnabVladimir extends CParTeploVodSnab{
    function __construct(){
        parent::__construct();
        $this->price_id = 11532584;
    }
}